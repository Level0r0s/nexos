...
/******************************************************
  ...
  *****************************************************
  ...
  $Source: /cvs/modules/Downloads_Pro/blocks/block-Featured_Downloads.php,v $
  $Revision: 1.5 $
  $Author: phoenix $
...
$a = 1;
$result = $db->sql_query("SELECT lid, title FROM ".$dl_mod_prefix."_downloads WHERE active=1 AND pick=1 ORDER BY title ASC LIMIT 0,10");
if ($db->sql_numrows($result) < 1) {
    $content = 'ERROR';
    return trigger_error('There are no editor\'s picks in '.$dl_mod, E_USER_WARNING);
}
