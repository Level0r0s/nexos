...
echo "<table border='0' cellpadding='0' cellspacing='10' width='100%'>"
    ...
    ."<td rowspan='18' valign='top' width='230'><table><tr><td colspan=3>".MapPic($data[$GSgame]['custom']['id'], $data[$GSgame]['mapname'], 0, "")."</td></tr><tr><td>";
		if (GetArrVal($data[$GSgame]['numplayers'])!=0){
			$cols[0] = array(_PLAYERNAME, _FRAGS, _DEATHS, _TEAM, _PING);
...
echo "</td></tr></table></td></tr>"
    ...
    if (GetArrVal($data[$GSgame]['numteams'])=='0') {
    	echo _NONE;
    } else {
    	echo GetArrVal($data[$GSgame]['numteams']);
    }
echo "</td></tr>"
    ."<tr><td>"._PASSWORD."</td><td>";
    if(GetArrVal($data[$GSgame]['passworded'])=='0') {
		echo _PASSREQN;
    } else {
    	echo _PASSREQY;
    }
...
echo "<tr><td colspan=\"2\" align=\"center\"><a href=\"http://www.painkillergame.com\" target=\"_blank\"><img border=\"0\" src=\"modules/".$module_name."/output_templates/".$data[$GSgame]['custom']['id']."/logo.png\"></a></td></tr></table>";
?>