...
echo "<table border='0' cellpadding='0' cellspacing='10' width='100%'>"
    ...
    ."<td rowspan='18' valign='top' width='230'><table><tr><td colspan=3>".MapPic($data[$GSgame]['custom']['id'], $data[$GSgame]['mapname'], 0, "")."</td></tr><tr><td>";
		if (GetArrVal($data[$GSgame]['numplayers'])!=0){
			$cols[0] = array(_PLAYERNAME, _TEAM, _DEATHS, _FRAGS, _PING);

			for ($p = 0; $p <= GetArrVal($data[$GSgame]['numplayers']) ; $p++){
				if (GetArrVal($data[$GSgame]['players'][$p]['player'])!="[Server]") {
...
echo "</td></tr>"
    ."<tr><td colspan=\"2\" align=\"center\"><a href=\"http://www.interplay.com/games/product.asp?GameID=31\" target=\"_blank\"><img border=\"0\" src=\"modules/".$module_name."/output_templates/".$data[$GSgame]['custom']['id']."/logo.png\"></a></td></tr></table>";
?>