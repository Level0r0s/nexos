...
if (isset($data[$GSgame]['Email'])){
	echo "<tr><td valign='top'>"._ADMININFO."</td><td valign='top'><a href=\"mailto:".GetArrVal($data[$GSgame]['Email'])."\">".GetArrVal($data[$GSgame]['Email'])."</a>";
	if (isset($data[$GSgame]['URL:'])){
      $Aurl = GetArrVal(strtolower($data[$GSgame]['URL:']));
		if(substr($Aurl, 0, 7)=="http://") {
		 echo "<br><a href=\"".$Aurl."\" target=\"_blank\">"._WWW."</a></td></tr>";
		} else {
		 echo "<br><a href=\"http://".$Aurl."\" target=\"_blank\">"._WWW."</a></td></tr>";
		}
	}
}
