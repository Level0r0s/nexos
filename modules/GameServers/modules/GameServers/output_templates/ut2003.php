    ...
    ."<table><tr><td colspan=4>".MapPic($data[$GSgame]['custom']['id'], $data[$GSgame]['map'], 0, "")."</td></tr><tr><td>";
		...
		$cols[0] = array(_PLAYERNAME, _PING, _FRAGS);
			for ($p = 0; $p <= $data[$GSgame]['num_players'] ; $p++){
				...
				$cols[$p+1][0] = "<a href=\"http://ut2003stats.epicgames.com/index.php?stats=players&SearchName=".GetArrVal($data[$GSgame]['players'][$p]['name'])."\" target=\"_blank\">".GetArrVal($data[$GSgame]['players'][$p]['name'])."</a>";
