...
/*
 * GameQ - UT2004 protocol (http://gameq.sf.net)
 * Copyright (C) 2003 Tom Buskens (tombuskens@users.sourceforge.net)
 * Modified for UT2004 by Neil Myers
 *
...
//dump all strings returned by server (uncomment for debug)
//echo "<pre>";
//print_r($data);
//echo "</pre>";
...
/* middle strings, misc vars */
for ($middle=1; $middle <= count($data)-1; $middle++) {
		$d = $data[$middle];
		$strlen = strlen($d);
...
/* last string, players (not bots */
if (isset($data[count($data)-1]) && strlen($data[count($data)-1]) > 10 && $output['num_players']!=0)
{
	$d = $data[count($data)-1];
	$strlen = strlen($d);
	$i=5;
