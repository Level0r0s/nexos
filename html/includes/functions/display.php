...
function viewbanner() {
	...
	global $prefix, $db;
	$result = $db->sql_query("SELECT * FROM ".$prefix."_banner WHERE type='0' AND active='1' ORDER BY RAND() LIMIT 1");
	...
	if ($imptotal <= $impmade && $imptotal != 0) {
		...
		$message = _HELLO." $to_name,\n\n"
			...
			._HOPEYOULIKED."\n\n"
			._THANKSUPPORT."\n\n"
			."- {$MAIN_CFG['global']['sitename']} "._TEAM."\n".$MAIN_CFG['global']['nukeurl'];
		send_mail($mailer_message, $message,0, "{$MAIN_CFG['global']['sitename']}: "._BANNERSFINNISHED, $row['user_email'], $row['username']);
		$db->sql_freeresult($result);
	}
	if ($textban) {
