...
//		$this->members[$this->user_id] = $member;
	...
	private function loginmember($visitor_ip)
	{
		...
		if ($setinfo['user_password'] != '' && $setinfo['user_level'] > 0) {
			...
			if ($setinfo['user_password'] != $pass) { URL::redirect(URL::index('Your_Account&error=2'), true); }
			if ($sec_code & 2) {
				$gfxid = isset($_POST['gfxid']) ? $_POST['gfxid'] : 0;
				$code = $CPG_SESS['gfx'][$gfxid];
				$gfx_check  = isset($_POST['gfx_check']) ? $_POST['gfx_check'] : '';
				if (strlen($gfx_check) < 2 || $code != $gfx_check) { URL::redirect(URL::index('Your_Account&error=2'), true); }
			}
			$db->sql_query('DELETE FROM '.$prefix."_session WHERE host_addr=$visitor_ip AND guest=1");
	...
	************************************************************************************/
	...
	public function setmemcookie($setuid=false, $setpass='', $secure=false)
	{
		global $MAIN_CFG;
		if (!$setuid) {
...
//			if ($this->local) setcookie($MAIN_CFG['cookie']['member'],$data,(time()+15552000), $MAIN_CFG['cookie']['path']);
			setcookie($MAIN_CFG['cookie']['member'],$data,(time()+15552000), $MAIN_CFG['cookie']['path'], $MAIN_CFG['cookie']['domain']); //, int secure
			return $data;
		}
	}
	...
	************************************************************************************/
	...
	public function setadmcookie($setaid=false, $id=0, $pwd='', $persistent=false, $secure=false)
	{
		global $MAIN_CFG;
		if (!$setaid) {
			...
			$time = ($persistent ? (time()+(86400*30)) : 0);
		}
