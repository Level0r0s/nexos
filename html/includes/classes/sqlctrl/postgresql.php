...
/*
  ...
  | GPL V2 or higher.                                                 |
  +-------------------------------------------------------------------+
  | index.php - NexOS postgresql driver file                          |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
class SQLCtrl extends DBCtrl {
	...
	public static function backup($database, $tables, $filename, $structure=true, $data=true, $drop=true, $compress=true, $full=false)
	{
		...
		$controls = "--$crlf-- PostgreSQL dump : $database$crlf"
			."-- "._ON." ".formatDateTime(time(), _DATESTRING)." !$crlf--$crlf$crlf"
	...
	public static function get_index($schema, $tables, $crlf, $compress)
	{
...
//				DBCtrl::output($list.$row[0].';', $compress);
	...
	public static function drop_table_struct($schema, $tables, $crlf, $compress)
	{
		global $db;
		foreach ($tables as $table) {
	...
	public static function get_table_struct($schema, $tables, $crlf, $compress)
	{
		global $db;
		foreach ($tables as $table) {
			$schema_create = $crlf.'--'.$crlf.'-- Table structure for table '.$table.$crlf.'--'.$crlf;
	...
	public static function get_table_content($schema, $tables, $crlf, $complete, $compress)
	{
		global $db;
		foreach ($tables as $table) {
			$str = $fields = '';
	...
	public static function get_function($schema, $crlf)
	{
		global $db;
	...
	public static function get_sequence($schema, $tables, $crlf, $full)
	{
		global $db;
		$list = '';
		if ($full) {
