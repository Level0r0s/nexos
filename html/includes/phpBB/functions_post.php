 ...
 *
 *	 $Id: functions_post.php,v 9.23 2009/05/27 22:57:36 phoenix Exp $
 *
 *
 ***************************************************************************/
/***************************************************************************
				...
				if (sizeof($bcc_list_ary)) {
					...
					while (list($user_lang, $bcc_list) = each($bcc_list_ary))
					{
						...
						$emailer->assign_vars(array(
							...
							'U_TOPIC' => URL::index('&file=viewtopic&' . POST_POST_URL . "=$post_id", true, true)."#$post_id",
							'U_STOP_WATCHING_TOPIC' => URL::index('&file=viewtopic&' . POST_TOPIC_URL . "=$topic_id&unwatch=topic", true, true))
						);
						$emailer->send();
						$emailer->reset();
		...
		if ($num_smilies) {
			...
			if ($num_smilies > $inline_rows * $inline_columns) {
				...
				$template->assign_vars(array(
					...
					'U_MORE_SMILIES' => URL::index("smilies&amp;field=message&amp;form=post")
					)
				);
			}
