 ...
 *
 *	 $Id: usercp_email.php,v 9.9 2008/08/03 04:58:49 nanocaiordo Exp $
 *
 *
 ***************************************************************************/
...
if ($row = $db->sql_ufetchrow("SELECT username, user_email, user_viewemail, user_lang FROM " . USERS_TABLE . " WHERE user_id = $user_id", SQL_ASSOC)) {
	...
	if ( $row['user_viewemail'] || $userdata['user_level'] == ADMIN ) {
		...
		if ( isset($_POST['submit']) ) {
			...
			if ( !$error ) {
				...
				if ( !empty($_POST['cc_email']) ) {
					...
					$message = $lang['Email_sent'] . '<br /><br />' . sprintf($lang['Click_return_index'],	'<a href="' . URL::index() . '">', '</a>');
		...
		$template->assign_vars(array(
			...
			'S_HIDDEN_FIELDS' => '',
			'S_POST_ACTION' => URL::index("&amp;file=profile&amp;mode=email&amp;" . POST_USERS_URL . "=$user_id"),
