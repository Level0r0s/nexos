 ...
 *
 *	 $Id: displaying.php,v 9.13 2009/05/27 22:57:35 phoenix Exp $
 *
 *
 ***************************************************************************/
...
function board_count_rebuild(array &$parent, array &$childs, &$unread_topics, $new_topic_data, $newparent=array()) {
$sub = !empty($newparent) ? array_shift($childs[$newparent['forum_id']]) : array_shift($childs[$parent['forum_id']]);
	if (!empty($sub)) {
		$parent['forum_posts'] += $sub['forum_posts'];
		$parent['forum_topics'] += $sub['forum_topics'];
		if (!$unread_topics) $unread_topics = !empty($new_topic_data[$sub['forum_id']]);
		if (isset($childs[$sub['forum_id']])) {
			board_count_rebuild($parent, $childs, $unread_topics, $new_topic_data, $sub);
		}
	}
}
