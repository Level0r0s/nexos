...
/*
  ...
  | GPL V2 or higher.                                                 |
  +-------------------------------------------------------------------+
  | block-CPG-Last_comments.php - NexOS Administration Backend file   |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
while ($row = $db->sql_fetchrow($result)) {
	...
	$content .= '<p style="text-align:center;"><a href="'.URL::index($cpg_dir. '&amp;file=displayimage&amp;meta=lastcom&amp;cat=0&amp;pos='.$pic).'"><img src="'.get_pic_url($row, 'thumb').'" alt="'.$thumb_title.'" title="'.$thumb_title.'" /><br />'.truncate_stringblocks($author, $auth_length).'</a> <br />'.truncate_stringblocks($messagebody, $body_length).'<br />('.$date.')';
	$pic++;
} 
