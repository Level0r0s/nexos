...
/*
  ...
  | GPL V2 or higher.                                                 |
  +-------------------------------------------------------------------+
  | block-Groups.php - NexOS Administration Backend file              |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
if (isset($userinfo['_mem_of_groups'])) {
	...
	foreach ($userinfo['_mem_of_groups'] as $id => $name) {
		$in_group[] = $id;
		if (!empty($name)){
			$s_member_groups .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>&#8226;</b>&nbsp;<a title="'.$name.'" href="'.URL::index('Groups&amp;g='.$id).'">'.$name.'</a><br />';
		}
	}
}
...
if (is_user()) {
	...
	if ($db->sql_numrows($result)) {
		...
		while ( $row = $db->sql_fetchrow($result) ) {
			...
			$s_pending_groups .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>&#8226;</b>&nbsp;<a title="'.$row['group_name'].'" href="'.URL::index('Groups&amp;g='.$row['group_id']).'">'.$row['group_name'].'</a><br />';
		}
	}
}
...
while ($row = $db->sql_fetchrow($result)) {
	if ($row['group_type'] != 2 || is_admin()) {
		$s_group_list .='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>&#8226;</b>&nbsp;<a title="'.$row['group_name'].'" href="'.URL::index('Groups&amp;g='.$row['group_id']).'">'.$row['group_name'].'</a><br />';
	}
}
