...
/*
  ...
  | GPL V2 or higher.                                                 |
  +-------------------------------------------------------------------+
  | block-Survey.php - NexOS Administration Backend file              |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
global $prefix, $MAIN_CFG, $currentlang, $db, $content, $userinfo, $ThemeSel;

$querylang = ($MAIN_CFG['global']['multilingual']) ? "AND planguage='$currentlang' OR planguage=''" : '';

$result = $db->sql_query("SELECT poll_id, poll_title, voters, anonymous FROM ".$prefix."_poll_desc WHERE artid=0 $querylang ORDER BY poll_id DESC LIMIT 1");
if ($db->sql_numrows($result) < 1) {
	...
	$content .= '<form action="'.URL::index("Surveys").'" method="post" enctype="multipart/form-data" accept-charset="utf-8">';
...
//				  }
	...
	<a href="'.URL::index("Surveys&amp;op=results&amp;pollid=$poll_id").'"><b>'._RESULTS.'</b></a> <b>::</b> <a href="'.URL::index('Surveys').'"><b>'._POLLS.'</b></a><br />
	<br />'._VOTES.": <b>$sum</b>\n";
