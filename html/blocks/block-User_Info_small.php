...
/*
  ...
  | GPL V2 or higher.                                                 |  
  +-------------------------------------------------------------------+
  | block-User_Info_small.php - NexOS Administration Backend file     |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
// number online
$result = $db->sql_query('SELECT COUNT(*), guest FROM '.$prefix.'_session 
	GROUP BY guest 
	ORDER BY guest');
...
if(is_user()) {
	...
	if (is_active('Private_Messages')) {
		...
		$content .= '&nbsp;<a title="'._READSEND.'" href="'.URL::index('Private_Messages').'"><img src="images/blocks/email.gif" alt="" /></a>&nbsp;&nbsp;<a title="'._READSEND.'" href="'.URL::index('Private_Messages').'">'._INBOX.'</a>';
...
<img src="images/blocks/ur-moderator.gif" alt="" /> '._BLATEST.': <a href="'.URL::index("Your_Account&amp;profile=$lastuser").'"><b>'.$lastuser.'</b></a><br />
