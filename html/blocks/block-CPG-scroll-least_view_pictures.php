...
/*
  ...
  | GPL V2 or higher.                                                 |
  +-------------------------------------------------------------------+
  | block-CPG-scroll-least_view_pictures.php                          |
...
while ($row = $db->sql_fetchrow($result)) {
	...
	$content .= '<a href="' . URL::index($cpg_dir . '&amp;file=displayimage&amp;album='.$row['aid'].'&amp;pos=' . $row["pid"]) . '"><img src="' . get_pic_url($row, 'thumb') . '" alt="' . $thumb_title . '" title="' . $thumb_title . '" /><br />' . $views_title . '</a><br /><br />';
} 
