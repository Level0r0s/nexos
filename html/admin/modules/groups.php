...
/*
  ...
  | GPL V2 or higher.                                                 |  
  +-------------------------------------------------------------------+
  | groups.php - NexOS Administration Backend file                    |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
if (isset($_GET['edit']) || isset($_POST['new'])) {
	...
	echo '<form action="'.URL::admin('groups').'" method="post" name="post" enctype="multipart/form-data" accept-charset="utf-8">
  <table border="0" cellpadding="3" cellspacing="1" class="forumline" style="margin:auto;">
	<tr>
	...
	if (isset($_POST['delete'])) {
		...
		$row = $db->sql_ufetchrow("SELECT auth_mod FROM " . AUTH_ACCESS_TABLE . " 
			WHERE group_id = " . $group_id, SQL_ASSOC);
