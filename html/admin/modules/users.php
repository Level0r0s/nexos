...
/*
  ...
  | GPL V2 or higher.                                                 |  
  +-------------------------------------------------------------------+
  | users.php - NexOS Administration Backend file                     |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
function main() {
	...
	if ($susrows > 0) echo '<tr><td><a href="'.URL::admin('&amp;show=sususers').'">'._SUSPENDUSERS.'</a></td><td>: '.$susrows.'</td></tr>';
	if ($delrows > 0) echo '<tr><td><a href="'.URL::admin('&amp;show=delusers').'">'._DELETEUSERS.'</a></td><td>: '.$delrows.'</td></tr>';
	echo '</table><br />';
...
if (isset($_POST['wait'])) {
  ...
  if ($_GET['mode'] == 'edit') {
	...
	echo '
	<table cellspacing="1" cellpadding="4" border="0" style="margin:auto;" class="forumline">
	...
	<input type="submit" name="usersubmit" value="Find a username" class="liteoption" onclick="window.open(\''.URL::index('Forums&amp;file=search&amp;mode=searchuser&popup=1&menu=1').'\', \'_phpbbsearch\', \'height=150,resizable=yes,width=300\');return false;" /></td></tr>
</table></form></fieldset>';
	}
	showfooter();
  }
	...
	echo open_form(URL::admin(), 'adduser', 0,' style="border:none"');
  ...
  <tr>
	...
	foreach ($CLASS['member']->admin AS $field => $val) {
		if ($field != 'radminsuper' && 0 === strpos($field, 'radmin')) {
  ...
  if ($_POST['mode'] == 'edit') {
	...
	if (can_admin()) {
		...
		if ($num > 0) {
			...
			$message = _SORRYTO." {$MAIN_CFG['global']['sitename']} "._HASPROMOTE;
			$subject = _ACCTPROMOTE;
			send_mail($mailer_message, $message, 0, _ACCTPROMOTE, $add_email);
			cpg_error(_USERPROMOTED, _PROMOTEUSER, URL::admin());
		}
	}
  }
} else {
