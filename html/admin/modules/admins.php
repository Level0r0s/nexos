...
/*
  ...
  | GPL V2 or higher.                                                 | 
  +-------------------------------------------------------------------+
  | admins.php - NexOS Administration Backend file                    |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
if (isset($_GET['mode']) && $_GET['mode'] == 'add') {
	...
	if (!preg_match('#[0-9]#', $_POST['add_pwd']) && !preg_match('#[a-z]#', $_POST['add_pwd']) && !preg_match('#[A-Z]#', $_POST['add_pwd'])) { cpg_error(_PASSWORD_MALFORMED, _CREATIONERROR); }
	$add_pwd = md5($_POST['add_pwd']);
	$fields = 'aid, email, pwd';
	$values = "'$_POST[add_aid]', '$_POST[add_email]', '$add_pwd'";
	$result = $db->sql_query('SELECT * FROM '.$prefix.'_admins LIMIT 1');
...
// Update administrator settings
	...
	if ($chng_pwd2 != '') {
		if (!preg_match('#[0-9]#', $chng_pwd) && !preg_match('#[a-z]#', $chng_pwd) && !preg_match('#[A-Z]#', $chng_pwd)) { cpg_error(_PASSWORD_MALFORMED); }
	...
	OpenTable();
	echo open_form(URL::admin('admins&amp;update='.$aid), false, _MODIFYINFO).'
  ...
  <table border="0" width="100%">
	<tr style="background:'.$bgcolor2.';"><td align="center"><strong>'._NAME.'</strong></td><td align="center"><strong>'._PERMISSIONS.'</strong></td><td align="center"><strong>'._FUNCTIONS.'</strong></td></tr>';
	...
	while ($row = $db->sql_fetchrow($result, SQL_ASSOC)) {
		...
		echo '</td><td>';
		if (can_admin() || $row['aid'] == is_admin()) echo '<a href="'.URL::admin('&amp;modify='.$row['admin_id']).'">'._MODIFYINFO.'</a>';
		if (can_admin() && $row['aid'] != is_admin()) echo ' / <a href="'.URL::admin('&amp;del_aid='.$row['admin_id']).'">'._DELAUTHOR.'</a>';
		echo '</td></tr>';
	}
	echo '</table>';
	CloseTable();
