...
/*
  ...
  | GPL V2 or higher.                                                 |
  +-------------------------------------------------------------------+
  | ranks.php - NexOS Administration Backend file                     |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
		...
		else if( $mode == "save" )
		{
			...
			for( $i = 0; $i < $rank_count; $i++)
			{
				...
				$template->assign_block_vars("ranks", array(
					...
					"U_RANK_EDIT" => URL::admin("$op&amp;mode=edit&amp;id=$rank_id"),
					"U_RANK_DELETE" => URL::admin("$op&amp;mode=delete&amp;id=$rank_id")
					)
				);
			}
		}
