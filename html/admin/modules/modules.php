...
/*
  ...
  | GPL V2 or higher.                                                 |
  +-------------------------------------------------------------------+
  | modules.php - NexOS Administration Backend file                   |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
if (isset($_GET['change'])) {
	...
	if (isset($_POST['confirm'])) {
		...
		cpg_delete_msg(URL::admin('&amp;home='.$mid), _SURETOCHANGEMOD.' <strong>'.$MAIN_CFG['global']['main_module'].'</strong> '._TO." <strong>$new_m</strong>?");
	}
	URL::redirect(URL::admin('modules'));
}
	...
	if (isset($editmodule)) {
		if ($version != $editmodule->version) $version .= ' <a href="'.URL::admin('&amp;upgrade='.$mid).'">'.sprintf(_UPGRADE, $editmodule->version).'</a>';
		$dbsize = 0;
		$backup = '';
...
//	if (is_dir('modules/'.$title.'/CVS')) {
	if (is_dir('modules/'.$title.'/CVS') && is_writeable('modules/'.$title)) {
		...
		<br /><a href="'.URL::admin('&amp;getcvs='.$mid).'">'._CVS_UPDATE.'</a>
		</fieldset>';
	}
	CloseTable();
}
	...
	if ($module->upgrade($version)) {
		...
		if ($module->radmin && !isset($_SESSION['CPG_ADMIN']['radmin'.strtolower($class)])) {
		...
		Cache::clear_conf();
		require('header.php');
		cpg_error(_TASK_COMPLETED, '', URL::admin('modules&edit='.$mid));
	}
	...
	if (!CVS::create($path, $_POST['server'], $_POST['folder'], $_POST['module'], $_POST['cvsusername'], $_POST['cvspassword'])) {
		...
		if (!isset($log['error'])) {
			if (!isset($log['notes'])) {
				...
				cpg_error(_TASK_COMPLETED, '', URL::admin('modules'));
			}
	...
	if (isset($_GET['install'])) {
		...
		$db->sql_query('INSERT INTO '.$prefix."_modules (title, uninstall, version) VALUES ('$class', 1, '".$module->version."')");
		...
		Cache::clear_conf();
		cpg_error("The module \"$class\" has been properly installed, have a blast using it!", 'Module Installed', URL::admin('modules'));
	...
	while ($file = readdir($handle)) {
		if (false === strpos($file, '.') && $file != 'CVS') {
			$class = "$file";
			if ($class != '' && !isset($mods[$class]) && file_exists('modules/'.$class.'/index.php')) {
	...
	while (list($title, $row) = each($allmods)) {
		...
		if ($row['title'] == $MAIN_CFG['global']['main_module']) {
			...
			$active = '<a href="'.URL::admin('&amp;change='.$mid).'"><img src="images/checked.gif" alt="'._ACTIVE.'" title="'._DEACTIVATE.'" border="0" /></a>';
		} else {
			$active = '<a href="'.URL::admin('&amp;change='.$mid).'"><img src="images/unchecked.gif" alt="'._INACTIVE.'" title="'._ACTIVATE.'" border="0" /></a>';
		}
		if ($row['view'] == 0) {
			$who_view = _MVALL;
		} elseif ($row['view'] == 1) {
		...
		if ($row['title'] == $MAIN_CFG['global']['main_module']) {
			...
			$change_status = ' <strong>::</strong> <a href="'.URL::admin('&amp;home='.$mid).'">'._PUTINHOME.'</a>';
		}
		if ($row['uninstall'] < 0) {
			echo "<tr$bgcolor><td></td><td>$moduledir</td><td colspan=\"3\">".$row['custom_title'].'</td><td><a href="'.URL::admin("modules&amp;install=$moduledir").'">Install</a></td></tr>';
		} else {
			if ($row['uninstall'] == 1) {
