...
/*
  ...
  | GPL V2 or higher.                                                 | 
  +-------------------------------------------------------------------+
  | index.php - NexOS Administration Backend file                     |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
OpenTable();
echo '<div style="text-align:center;"><span class="genmed"><strong>'._WHOSONLINE.'</strong></span><br />
...
OpenTable();
echo '<div style="text-align:center;"><span class="genmed"><strong>'._DEFHOMEMODULE.'</strong></span><br /><br /><img src="images/home.gif" alt="" /><br /><br />[ <strong>'.str_replace('_', ' ', $MAIN_CFG['global']['main_module']).'</strong> ]';
if (can_admin()) { echo '&nbsp;&nbsp;[ <a href="'.URL::admin('modules').'">'._CHANGE.'</a> ]'; }
echo '</div>';
CloseTable();
echo '<br />';

if (is_active('Surveys') && can_admin('surveys')) {
	list($poll_id, $poll_title) = $db->sql_ufetchrow("SELECT poll_id, poll_title FROM ".$prefix."_poll_desc WHERE artid=0 ORDER BY poll_id DESC", SQL_NUM);
	OpenTable();
	echo '<div style="text-align:center;"><span class="genmed"><strong>'._CURRENTPOLL.'</strong></span><br /><br />'.$poll_title;
	echo '<br /><br />[ <a href="'.URL::admin('Surveys&amp;mode=edit&amp;id='.$poll_id).'">'._EDIT.'</a> | <a href="'.URL::admin('Surveys&amp;mode=add').'">'._ADD.'</a> ]';
	echo '</div>';
	CloseTable();
	echo '<br />';
}
...
echo '<form action="'.URL::admin().'" method="post" accept-charset="utf-8">
