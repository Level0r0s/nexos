...
/*
  ...
  | GPL V2 or higher.                                                 |
  +-------------------------------------------------------------------+
  | newsletters.php - NexOS Administration Backend file               |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
if (isset($_POST['discard'])) {
	...
	$content = _HELLO.",\n\n$content\n\n\n"._NL_REGARDS.",\n\n{$MAIN_CFG['global']['sitename']} "._STAFF."\n\n\n\n"._NLUNSUBSCRIBE;
	$recipients = array();
	$limit = 1000; //$MAIN_CFG['email']['limit']
	set_time_limit(0);
	for ($offset=0; $offset<$count; $offset+=$limit) {
		$result = $db->sql_query($query." LIMIT $limit OFFSET $offset");
	...
	if (count($recipients) > 50) {
		while ($part_recips = array_splice($recipients,0,50)) {
			send_mail($mailer_message, $content, 1, $subject, $part_recips, '', $MAIN_CFG['global']['adminmail'], $MAIN_CFG['global']['sitename']);
		}
	} else {
		send_mail($mailer_message, $content, 1, $subject, $recipients, '', $MAIN_CFG['global']['adminmail'], $MAIN_CFG['global']['sitename']);
	}
/*
...
if (isset($_POST['preview'])) {
	...
	if (false !== stripos($content, '<br')) $content = nl2br($content);
	$preview = '<tr>
