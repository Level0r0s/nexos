...
/*
  ...
  | GPL V2 or higher.                                                 | 
  +-------------------------------------------------------------------+
  | info.php - NexOS Administration Backend file                      |
  | Authors:                                                          |
  |  Steven Sheeley                                                   |
  |  Richard R. Pufky                                                 |
...
if ($MAIN_CFG['global']['admingraphic'] == 0)  $MAIN_CFG['global']['admingraphic'] = 1;
...
function get_phpinfo($mode) {
	...
	$cache = str_replace('<th>', '<td class="infohead">', $cache[1]);
	$cache = str_replace('</th>', '</td>', $cache);
	return $cache;
}
...
OpenTable();
echo (($info == 'General') ? '<strong>General</strong>' : '<a href="'.URL::admin('info&amp;general').'">General</a>').' |
'.(($info == 'PHP Core') ? '<strong>PHP Core</strong>' : '<a href="'.URL::admin('info&amp;core').'">PHP Core</a>').' |
'.(($info == 'PHP Environment') ? '<strong>PHP Environment</strong>' : '<a href="'.URL::admin('info&amp;envi').'">PHP Environment</a>').' |
'.(($info == 'PHP Modules') ? '<strong>PHP Modules</strong>' : '<a href="'.URL::admin('info&amp;mods').'">PHP Modules</a>').' |
'.(($info == 'PHP Variables') ? '<strong>PHP Variables</strong>' : '<a href="'.URL::admin('info&amp;vars').'">PHP Variables</a>').' |
'.(($info == 'SQL Server') ? '<strong>SQL Server</strong>' : '<a href="'.URL::admin('info&amp;database').'">SQL Server</a>');
CloseTable();
OpenTable();
...
if (isset($_GET['mods'])) {
	...
	$cache = str_replace('<th colspan="2">', '<td class="infohead" colspan="2">', $cache);
	$cache = preg_replace('#(<div class="center">|<\/div>)#', '', $cache);
	$cache = str_replace('<h2>', '<div class="genmed"><strong>', $cache);
	$cache = str_replace('</h2>', '</strong></div>', $cache);
	$cache = preg_split('/(<table border="0" cellpadding="3" width="600">|<\/table><br>|<\/table><br \/>)/', $cache, -1, PREG_SPLIT_NO_EMPTY);
	for ($i=0; $i<count($cache); $i++) {
		if ($cache[$i] != '') {
			if (false !== strpos($cache[$i], '<div')) {
				echo '<hr/>'.$cache[$i];
			} else {
