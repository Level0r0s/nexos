...
/*																	  */
/*   $Id: admin_avatar.php,v 10.0 2010/11/05 00:56:57 djmaze Exp $	  */
/*																	  */
/************************************************************************/
...
switch( $mode )
{
	case "delete":
		...
		if ( unlink($real_avatar_dir.'/'.$target) ) {
			print "<tr><td>Success, $target deleted!</td></tr><tr><td><a href=\"".URL::admin("&amp;do=avatar")."\">Continue</a></td></tr></table>";
		} else {
			...
			while ($file = readdir($avatar_dir)) {
				...
				if( $file != "." && $file != ".." && preg_match("#\.(gif|jpg|jpeg|png)$#",$file) ) {
					...
					if (isset($avatar_usage[$file]) ) {
						...
						$edit_url = '<a href="'.URL::admin("users&amp;mode=edit&amp;edit=avatar&amp;id=$av_uid").'">'._EDITUSER.'</a>';
					} else {
						// Not used, safe to display delete link for admin
						$username = _NONE;
						$edit_url = '<a href="'.URL::admin("&amp;do=avatar&amp;mode=delete&target=$file").'" onClick="if(confirm(\'Are you sure you want to delete: '.$file.' ?\')) return true; else return false;">'._DELETE.'</a>';
					}
