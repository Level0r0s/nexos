 ...
 *
 *	  $Id: admin_attach_cp.php,v 10.0 2010/11/05 00:56:57 djmaze Exp $
 *
 ***************************************************************************/
...
$template->assign_vars(array(
	...
	'S_MODE_ACTION' => URL::admin('&amp;do=attach_cp'))
);
	...
	if ( $num_members > 0 ) {
		...
		for ($i = 0; $i < count($members); $i++) {
			...
			$template->assign_block_vars('memberrow', array(
				...
				'U_VIEW_MEMBER' => URL::admin('&amp;do=attach_cp&amp;view=attachments&amp;uid=' . $members[$i]['user_id'])
				)
			);
		}
	}
	...
	if (count($attachments) > 0)
	{
		for ($i = 0; $i < count($attachments); $i++)
		{
			...
			$template->assign_block_vars('attachrow', array(
				...
				'U_VIEW_ATTACHMENT' => URL::index('&amp;file=download&amp;id=' . $attachments[$i]['attach_id'])
//				  'U_VIEW_POST' => ($attachments[$i]['post_id'] != 0) ? URL::index("&amp;file=viewtopic&amp;" . POST_POST_URL . "=" . $attachments[$i]['post_id']) . "#" . $attachments[$i]['post_id']) : ''
				)
			);
