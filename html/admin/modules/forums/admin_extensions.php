 ...
 *
 *	 $Id: admin_extensions.php,v 10.0 2010/11/05 00:56:57 djmaze Exp $
 *
 ***************************************************************************/
...
if ($submit && $mode == 'extensions')
{
	...
	if (!$error) {
		$message = $lang['Attach_config_updated'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="'.URL::admin("&amp;do=extensions&amp;mode=extensions").'">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="'.URL::admin("forums").'">', '</a>');
...
if ($mode == 'extensions') {
	...
	$template->assign_vars(array(
		...
		'S_CANCEL_ACTION' => URL::admin("&amp;do=extensions&amp;mode=extensions"),
		'S_ATTACH_ACTION' => URL::admin("&amp;do=extensions&amp;mode=extensions"))
	);
...
if ($mode == 'groups') {
	...
	$template->assign_vars(array(
		...
		'S_CANCEL_ACTION' => URL::admin("&amp;do=extensions&amp;mode=groups"),
		'S_ATTACH_ACTION' => URL::admin("&amp;do=extensions&amp;mode=groups"))
	);
	...
	for ($i = 0; $i < $num_extension_group; $i++) {
		...
		$template->assign_block_vars('grouprow', array(
			...
			'U_VIEWGROUP' => ( $viewgroup == $extension_group[$i]['group_id'] ) ? URL::admin("&amp;do=extensions&amp;mode=groups") : URL::admin("&amp;do=extensions&amp;mode=groups&amp;" . POST_GROUPS_URL . "=" . $extension_group[$i]['group_id']),
			'U_FORUM_PERMISSIONS' => URL::admin("&amp;do=extensions&amp;mode=$mode&amp;e_mode=perm&amp;e_group=" . $extension_group[$i]['group_id']))
		);
...
if ($submit && $mode == 'forbidden') {
	...
	if (!$error) {
		$message = $lang['Attach_config_updated'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="'.URL::admin("&amp;do=extensions&amp;mode=forbidden").'">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="'.URL::admin("forums").'">', '</a>');
		message_die(GENERAL_MESSAGE, $message);
	}
...
if ($mode == 'forbidden') {
	...
	$template->assign_vars(array(
		'S_ATTACH_ACTION' => URL::admin("&amp;do=extensions&amp;mode=forbidden"),
		'S_HIDDEN_FIELDS' => '',
...
if (($e_mode == 'perm') && ($group != -1)) {
	...
	$template->assign_vars(array(
		...
		'L_RESET' => $lang['Reset'],
		'S_HIDDEN_FIELDS' => '',
		'A_PERM_ACTION' => URL::admin("&amp;do=extensions&amp;mode=groups&amp;e_mode=perm&amp;e_group=$group"))
	);
