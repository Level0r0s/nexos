...
if(isset($_GET['remove'])) {
	...
	if ( !empty($icon_id) ) {
		...
		$template->assign_vars(array(
			...
			'S_ACTION' => URL::admin("&amp;do=topic_icons"),
		...
		for($j = 0; $j < $total_forums; $j++) {
			...
			if ($forum_rows[$j]['cat_id'] == $cat_id) {
				$template->assign_block_vars("catrow.forumrow",	   array(
					...
					'U_VIEWFORUM' => URL::index("&amp;file=viewforum&amp;" . POST_FORUM_URL . "=$forum_id"),
					'FORUM_ID' => $forum_id)
				);
				...
				while( list($key, $val) = each($topic_icons) ) {
					...
					'U_REMOVE_ICON' => URL::admin("&amp;do=topic_icons&amp;remove=true&amp;id=" . $val['icon_id']))
					);
				}
...
while( list($key, $val) = each($topic_icons) )
{
	$template->assign_block_vars('globalicon', array(
		'ICON_SRC' => $val['icon_url'],
		'U_REMOVE_GLOBAL_ICON' => URL::admin("&amp;do=topic_icons&amp;remove=true&amp;id=" . $val['icon_id']))
	);
}
