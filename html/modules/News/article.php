...
$cpgtpl->assign_vars(array(
	...
	'NEWS_ADMIN'	  => (can_admin('News') ? '<a href="'.URL::admin('&amp;edit='.$sid).'">'._EDIT.'</a> | <a href="'.URL::admin('&amp;del='.$sid).'">'._DELETE.'</a>' : ''),
	'S_AUTHOR'	  => $story['aid'],
...
if ($story['associated'] != '') {
	...
	while ($atop = $db->sql_fetchrow($result)) {
		...
		$assoc .= '<a href="'.URL::index('&amp;topic='.$atop['topicid']).'"><img src="'.$atop['topicimage'].'" style="padding-right:10px;" alt="'.$atop['topictext'].'" title="'.$atop['topictext'].'" /></a>';
	}
	$db->sql_freeresult($result);
	if ($assoc != '') {
		$cpgtpl->assign_vars(array(
...
if (is_active('Surveys') && $story['haspoll']) {
	...
	$content .= '<br /><a href="'.URL::index('Surveys&amp;op=results&amp;pollid='.$poll_id).'"><b>'._RESULTS.'</b></a> <b>::</b> <a href="'.URL::index('Surveys').'"><b>'._POLLS.'</b></a><br />';
	$content .= '<br />'._VOTES.': <b>'.$sum.'</b>';
	if ($MAIN_CFG['global']['pollcomm']) {
		list($numcom) = $db->sql_ufetchrow("SELECT COUNT(*) FROM ".$prefix."_pollcomments 
...
$result = $db->sql_query("SELECT name, url FROM ".$prefix."_related 
	WHERE tid=$story[topic]");
...
list($topstory, $ttitle) = $db->sql_ufetchrow("SELECT sid, title FROM ".$prefix."_stories 
	WHERE topic=$story[topic] $querylang 
	ORDER BY counter DESC");
$content .= '<b>&#8226;</b>&nbsp;<a href="'.URL::index('&amp;topic='.$story['topic']).'">'._MOREABOUT.' '.$story['topictext'].'</a>
</span><br /><hr style="width:95%;" /><div style="text-align:center;" class="content"><b>'._MOSTREAD.' '.$story['topictext'].':</b><br />
<a href="'.URL::index('&amp;file=article&amp;sid='.$topstory).'">'.$ttitle.'</a></div>';
$block = array (
	'bid' => 10002,
	'view' => 0,
	'side' => 'r',
