...
if ((isset($_POST['postreply']) || isset($_POST['preview']) || isset($_GET['reply']) || isset($_GET['comment'])) &&
		(!$MAIN_CFG['global']['articlecomm'] || $db->sql_count($prefix.'_stories', "sid={$sid} AND acomm=0")))
{
...
elseif (isset($_POST['preview'])) {
	...
	if ($MAIN_CFG['global']['multilingual']) $querylang = "AND $querylang";
	...
	if ($topic > 0) {
		...
		if ($db->sql_numrows($result_a) < 1) {
			$cpgtpl->assign_block_vars('newsempty', array(
				...
				'U_NEWSINDEX' => URL::index('News'),
				'U_TOPICS'    => URL::index('Topics')
			));
		} else {
...
//	$result = $db->sql_query($sql.' LIMIT '.$storynum.' OFFSET 0');
	...
	while ($row = $db->sql_fetchrow($result, SQL_ASSOC)) {
		...
		$cpgtpl->assign_block_vars('newstopic', array(
			...
			'S_WRITES'    => _WRITES,
			'S_SID'       => $row['sid'],
			'U_NEWTOPIC'  => URL::index("News&amp;topic=$row[topic]")
		));
	}
