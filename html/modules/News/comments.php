...
function navbar($sid, $title, $thold, $mode, $order) {
	...
	$cpgtpl->assign_vars(array(
		...
		'S_BGCOLOR1' => $bgcolor1,
		'S_BGCOLOR2' => $bgcolor2,
		'U_ARTICLE'	 => URL::index('&amp;file=article&amp;sid='.$sid),
		'U_TOPREPLY' => URL::index('&amp;reply=0&amp;sid='.$sid),
		...
		while ($row = $db->sql_fetchrow($result)) {
			if($row['score'] >= $thold) {
				...
				$cpgtpl->assign_block_vars('comment', array(
					...
					'S_USERPM'     => _SENDAMSG,
					'U_USERPM'     => ((isset($usrdata) && is_active('Private_Messages')) ? URL::index("Private_Messages&amp;mode=post&amp;u=$usrdata[user_id]") : ''),
					'U_COMMENTEXT' => (isset($commentext) ? URL::index("&amp;comment=$tid&amp;sid=$sid") : ''),
					'U_DELETE'     => is_admin() ? URL::admin('comments&amp;del='.$tid) : '',
					'U_PARENT'     => ($pid != 0) ? URL::index("&amp;comment=$parentid&amp;sid=$sid") : '',
					'U_REPLY'      => ($MAIN_CFG['global']['anonpost'] || is_user()) ? URL::index("&amp;reply=$tid&amp;sid=$sid") : '',
				));
				++$comments;
		...
		while ($row = $db->sql_fetchrow($result)) {
			if($row['score'] >= $thold) {
				...
				$cpgtpl->assign_block_vars('comment', array(
					...
					'IS_FIRST_END' => false,
					'IS_LIST'    => true,
					'U_READCOMM' => URL::index("&amp;comment=$row[tid]&amp;sid=$row[sid]&amp;pid=$row[pid]")."#$row[tid]",
...
function DisplayComments($sid, $title, $pid=0, $tid=0) {
	...
	while ($row = $db->sql_fetchrow($something)) {
		...
		$cpgtpl->assign_block_vars('comment', array(
			...
			'S_USERPM'     => _SENDAMSG,
			'U_USERPM'     => ((isset($usrdata) && is_active('Private_Messages')) ? URL::index("Private_Messages&amp;mode=post&amp;u=$usrdata[user_id]") : ''),
			'U_COMMENTEXT' => (isset($commentext) ? URL::index("&amp;file=comments&amp;sid=$sid&amp;tid=$tid") : ''),
			'U_DELETE'     => is_admin() ? URL::admin("comments&amp;del=$tid") : '',
			'U_PARENT'     => ($pid != 0) ? URL::index("&amp;comment=0&amp;sid=$sid&amp;pid=$parentid") : '',
			'U_REPLY'      => ($MAIN_CFG['global']['anonpost'] || is_user()) ? URL::index("&amp;reply=$tid&amp;sid=$sid") : '',
		));
...
function singlecomment($tid, $sid) {
	...
	$cpgtpl->assign_vars('single', array(
		'U_REPLY' => ($MAIN_CFG['global']['anonpost'] || is_admin() || is_user()) ? '<a href="'.URL::index("&amp;reply=$tid&amp;sid=$sid").'">'._REPLY.'</a> | <a href="'.URL::index('&amp;file=article&amp;sid='.$sid).'">'._ROOT.'</a>' : '',
		...
		'S_MODERATE3'	=> ($MAIN_CFG['global']['moderate']) ? modthree($sid) : '',
	));
