...
{
	global $user_prefix, $db, $DeniedUserNames, $CensorList;
	if (strlen($username) > 50) { throw new Exception(_NICK2LONG); }
	if (strlen($username) < 4)  { throw new Exception(_NICK2SHORT); }
	if (is_numeric($username))  { throw new Exception(_NICKNUMERIC); }
	if (preg_match('#(\ |\*|#|\\\|%|"|\'|`|&|\^|@)#',$username))
	{
		throw new Exception(_ERRORINVNICK);
	}

	$deniedusers = implode('|', array_merge(array('staff'), $DeniedUserNames, $CensorList));
	$words = array();
	if (preg_match('#('.$deniedusers.')#', $username, $words))
	{
		throw new Exception(_NAMEDENIED." <b>\"{$words[0]}\"</b>");
	}
...
function member_block()
{
	...
	$content = '<span class="gen">'._TB_INFO.'</span><div style="margin-left: 8px;">'
		.((isset($_GET['profile']) && $_GET['profile'] == $userinfo['user_id']) ? '<b>'._TB_PROFILE_INFO.'</b>' : '<a href="'.URL::index('Your_Account&amp;profile='.$userinfo['user_id']).'">'._TB_PROFILE_INFO.'</a>').'<br />'
		.(($mode == 'profile') ? '<b>'._TB_EDIT_PROFILE.'</b>' : '<a href="'.URL::index('Your_Account&amp;edit=profile').'">'._TB_EDIT_PROFILE.'</a>').'<br />'
		.(($mode == 'private') ? '<b>'._TB_EDIT_PRIVATE.'</b>' : '<a href="'.URL::index('Your_Account&amp;edit=private').'">'._TB_EDIT_PRIVATE.'</a>').'<br />'
		.(($mode == 'reg_details') ? '<b>'._TB_EDIT_REG.'</b>' : '<a href="'.URL::index('Your_Account&amp;edit=reg_details').'">'._TB_EDIT_REG.'</a>').'<br />'
	.'</div><span class="gen">'._TB_CONFIG.'</span><div style="margin-left: 8px;">'
		.(($mode == 'prefs') ? '<b>'._TB_EDIT_PREFS.'</b>' : '<a href="'.URL::index('Your_Account&amp;edit=prefs').'">'._TB_EDIT_PREFS.'</a>').'<br />'
		.(($op == 'edithome') ? '<b>'._TB_EDIT_HOME.'</b>' : '<a href="'.URL::index('Your_Account&amp;op=edithome').'">'._TB_EDIT_HOME.'</a>').'<br />'
		.(($op == 'editcomm') ? '<b>'._TB_EDIT_COMM.'</b>' : '<a href="'.URL::index('Your_Account&amp;op=editcomm').'">'._TB_EDIT_COMM.'</a>').'<br />'
	...
	if (($MAIN_CFG['avatar']['allow_local']) || ($MAIN_CFG['avatar']['allow_remote'])) {
		$content .= (($mode == 'avatar') ? '<b>'._TB_EDIT_AVATAR.'</b>' : '<a href="'.URL::index('Your_Account&amp;edit=avatar').'">'._TB_EDIT_AVATAR.'</a>').'<br />';
	}
	if (is_active('coppermine')) {
		$content .= '<a href="'.URL::index('coppermine&amp;cat='.(10000+$userinfo['user_id'])).'">'._TB_PERSONAL_GALLERY.'</a><br />';
	}
	if (is_active('Blogs')) {
		$content .= '<a href="'.URL::index('Blogs&amp;mode=add').'">'._TB_PERSONAL_JOURNAL.'</a><br />';
	}
	$content .= '</div>';
	...
	OR (privmsgs_from_userid='.$userinfo['user_id'].' AND privmsgs_type = 4)', SQL_NUM);
		...
		$content .= '<span class="gen">'._TB_PRIVMSGS.'</span><div style="margin-left: 8px;">'
			.(($folder == 'inbox') ? '<b>'._TB_PRIVMSGS_INBOX.': '.$pm_inbox.'</b>' : '<a href="'.URL::index('Private_Messages&amp;folder=inbox').'">'._TB_PRIVMSGS_INBOX.': <b>'.$pm_inbox.'</b></a>').'<br />'
			.(($folder == 'outbox') ? '<b>'._TB_PRIVMSGS_OUTBOX.': '.$pm_outbox.'</b>' : '<a href="'.URL::index('Private_Messages&amp;folder=outbox').'">'._TB_PRIVMSGS_OUTBOX.': <b>'.$pm_outbox.'</b></a>').'<br />'
			.(($folder == 'sentbox') ? '<b>'._TB_PRIVMSGS_SENTBOX.': '.$pm_sentbox.'</b>' : '<a href="'.URL::index('Private_Messages&amp;folder=sentbox').'">'._TB_PRIVMSGS_SENTBOX.': <b>'.$pm_sentbox.'</b></a>').'<br />'
			.(($folder == 'savebox') ? '<b>'._TB_PRIVMSGS_SAVEBOX.': '.$pm_savebox.'</b>' : '<a href="'.URL::index('Private_Messages&amp;folder=savebox').'">'._TB_PRIVMSGS_SAVEBOX.': <b>'.$pm_savebox.'</b></a>').'<br />'
			.((isset($_GET['mode']) && $_GET['mode'] == 'post') ? '<b>'._TB_PRIVMSGS_SEND.'</b>' : '<a href="'.URL::index('Private_Messages&amp;mode=post').'">'._TB_PRIVMSGS_SEND.'</a>').'<br />'
			.'</div>';
	}
	$content .= '<span class="gen">'._TB_TASKS.'</span><div style="margin-left: 8px;">
		<a href="'.URL::index('Your_Account&amp;op=logout').'">'._ACCTEXIT.'</a></div>';
