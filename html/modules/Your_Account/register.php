...
{
	if (empty($_POST['validate_username']) || !isset($_SERVER['REQUEST_METHOD']) || 'POST' != $_SERVER['REQUEST_METHOD']) {
...
if (Security::check_post()) {
	...
	$modheader .= '<script type="text/javascript" src="modules/Your_Account/register.js"></script>'."\n";
	require_once('header.php');
	register_form();
}
...
function activate($uid, $check_num) {
	...
	if (!$user_cfg['requireadmin']) {
		$db->sql_query('DELETE FROM '.$user_prefix.'_users_temp WHERE time < '.(time()-86400));
	}
