...
if (!$confirm) {
	...
	cpg_delete_msg(URL::index('&amp;folder='.$folder),
	...
	if (count($mark_list)) {
		...
		switch( $folder ) {
			case 'inbox':
				$delete_sql .= "privmsgs_to_userid = ".$userinfo['user_id']." AND (
					privmsgs_type = ".PM_READ_MAIL." OR privmsgs_type = ".PM_NEW_MAIL." OR privmsgs_type = ".PM_UNREAD_MAIL." )";
				break;
			case 'outbox':
				$delete_sql .= "privmsgs_from_userid = ".$userinfo['user_id']." AND (
					privmsgs_type = ".PM_NEW_MAIL." OR privmsgs_type = ".PM_UNREAD_MAIL." )";
				break;
			case 'sentbox':
				$delete_sql .= "privmsgs_from_userid = {$userinfo['user_id']} AND privmsgs_type = ".PM_SENT_MAIL;
				break;
			case 'savebox':
				$delete_sql .= "( ( privmsgs_from_userid = {$userinfo['user_id']}
					AND privmsgs_type = ".PM_SAVED_OUT_MAIL." )
				OR ( privmsgs_to_userid = {$userinfo['user_id']}
					AND privmsgs_type = ".PM_SAVED_IN_MAIL." ) )";
				break;
