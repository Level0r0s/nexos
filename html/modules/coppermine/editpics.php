...
function form_pic_info($text)
{
	...
	$thispid = $CURRENT_PIC['pid'];
			$thumb_link = '<a href="'.URL::index("&amp;file=displayimagepopup&amp;pid=$thispid&amp;fullsize=1")."\" target=\"" . uniqid(rand()) . "\" onClick=\"MM_openBrWindow('".URL::index("&amp;file=displayimagepopup&amp;pid=$thispid&amp;fullsize=1")."','" . uniqid(rand()) . "','toolbar=yes,status=yes,resizable=yes,scrollbars=yes,width=$winsizeX,height=$winsizeY');return false\">";
...
if (UPLOAD_APPROVAL_MODE) {
	...
	$form_target = URL::index('&amp;file=editpics&amp;mode=upload_approval&amp;start=' . $start . '&amp;count=' . $count);
	$title = UPL_APPROVAL;
} else {
	...
	$form_target = URL::index('&amp;file=editpics&amp;mode=upload_approval&amp;album=' . $album_id . '&amp;start=' . $start . '&amp;count=' . $count);
	$title = EDIT_PICS;
} 
...
echo '
	...
	<tr>
		<td class="tableb" colspan="3" align="center">
			...
			<select onchange="if(this.options[this.selectedIndex].value) window.location.href=\''.URL::index('',0,1).'&file=editpics&mode=upload_approval&album='.$album_id.'&start='.$start.'&count=\'+this.options[this.selectedIndex].value;"  name="count" class="listbox">
