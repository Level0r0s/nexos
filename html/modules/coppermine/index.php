...
function list_users()
{
  ...
  $sql = "SELECT user_id, username, user_avatar as avatar, user_avatar_type, a.title, " .
         ...
         "GROUP BY user_id, username, user_avatar, user_avatar_type, a.title ORDER BY username";
  ...
  for ($i = 0; $i < count($rowset); $i++) {
    ...
    $params = array(
      ...
      '{USER_PROFILE_LINK}' => URL::index("Your_Account&amp;profile=".$user['user_id']),
      '{ALBUMS}' => $albums_txt,
    ...
    $user_list[] = array(
      ...
      'image' => $user_thumb,
      'caption' => $caption,
      'url' => URL::index("&amp;cat=".(FIRST_USER_CAT + $user['user_id'])),
    );
  } 
...
function list_cat_albums($cat = 0, $buffer = true)
{
  ...
  */
  ...
  $sql = "SELECT a.aid, a.title, a.description, visibility, filepath, " .
         ...
         "LIMIT " . ($upper_limit - $lower_limit) . " OFFSET " . $lower_limit;
