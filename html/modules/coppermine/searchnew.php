...
if (isset($_POST['insert'])) {
	...
	foreach ($_POST['pics'] as $pic_id) {
		...
		$status = '<a href="'.URL::index($module_name.'&amp;file=addpic&amp;aid='.$album_id. '&amp;pic_file=' . ($_POST['picfile_' . $pic_id]) . '&amp;reload=' . uniqid(''),0 ). '"><img src="'.URL::index($module_name.'&amp;file=addpic&amp;aid='.$album_id.'&amp;pic_file=' . ($_POST['picfile_' . $pic_id]) . '&amp;reload=' . uniqid(''),false). '" class="thumbnail" border="0" width="24" height="24" alt="" /><br /></a>';
		$album_name = $album_array[$album_id];
...
elseif (isset($_GET['startdir'])) {
	...
	if (false !== strpos($startdir, '..')) die('Access denied: '.$startdir); // thanks to waraxe for finding this admin vulnerability
	pageheader(PAGE_TITLE);
	starttable("100%");
	$action=URL::index("&amp;file=searchnew&amp;insert=1");
	echo '
