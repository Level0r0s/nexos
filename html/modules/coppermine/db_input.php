...
switch ($event) {
	...
	case 'comment_update':
		...
		if (!USER_IS_ADMIN) {
			$redirect = URL::index("$module_name&file=displayimage&pid=" . $GET['pid']);
	...
	case 'comment':
		...
		if (!USER_ID) { // Anonymous users, we need to use META refresh to save the cookie
			$insert = $db->sql_query("INSERT INTO {$CONFIG['TABLE_COMMENTS']} (pid, msg_author, msg_body, msg_date, author_md5_id, author_id, msg_raw_ip, msg_hdr_ip) VALUES ('$pid', '$msg_author', '$msg_body', ".time().", '{$USER['ID']}', '0', '$raw_ip', $hdr_ip)");
			$USER['name'] = $_POST['msg_author'];
			$redirect = URL::index("$module_name&file=displayimage&pid=" . $pid);
			$host =  $MAIN_CFG['server']['domain'].$MAIN_CFG['server']['path'];
			...
			$insert = $db->sql_query("INSERT INTO {$CONFIG['TABLE_COMMENTS']} (pid, msg_author, msg_body, msg_date, author_md5_id, author_id, msg_raw_ip, msg_hdr_ip) VALUES ('$pid', '" . CPG_USERNAME . "', '$msg_body', ".time().", '', '" . USER_ID . "', '$raw_ip', $hdr_ip)");
			$redirect = URL::index("$module_name&file=displayimage&pid=" . $pid);
			$host =  $MAIN_CFG['server']['domain'].$MAIN_CFG['server']['path'];
			if ($CONFIG['comment_email_notification']) {
				$mail_body = $msg_body . "\n\r ".COM_ADDED." @ http://" . $host  .''. $redirect;
	...
	case 'album_update':
		...
		$redirect = URL::index("&cat=$category");
		URL::redirect($redirect, 1);
		break;
	...
	case 'picture':
		...
		if (!preg_match('#^[a-zA-Z0-9_\-]+$#', $picname)) {
			$picname = preg_replace('#á|à|a|a|a|â|ã|ä#', 'a', $picname);
			$picname = preg_replace('#é|è|æ|ë|ê|?#', 'e', $picname);
			$picname = preg_replace('#ì|í|î|ï|?|?#', 'i', $picname);
			$picname = preg_replace('#ð|ò|ó|ô|õ|ö|ø|œ|o|?#', 'o', $picname);
			$picname = preg_replace('#ù|ú|û|ü|?#', 'u', $picname);
			$picname = preg_replace('#ý|þ|ÿ|?#', 'y', $picname);
			$picname = preg_replace('#c|c|c|c|ç#', 'c', $picname);
