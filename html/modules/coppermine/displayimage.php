...
function html_picture_menu($id)
{
	...
	<form method="post" action="'.URL::index("&amp;file=editOnePic",1,1).'">
		<input type="hidden" name="id" value="'.$id.'" />
		<input name="submit" title="'.EDIT_PIC.'" type="image" src="' . $THEME_DIR . '/images/edit.gif" /></form></span>' . "\n".'
		<span style="float:right;"><form method="post" action="'.URL::index("&amp;file=delete",1,1).'" enctype="multipart/form-data" accept-charset="utf-8">
	...
	/*<br /><div style="text-align:center;" class="admin_menu">
	<a href="'.URL::index("&amp;file=editOnePic&amp;id=$id&amp;what=picture").'" class="admin_menu">'.EDIT_PIC.'</a>
	<a href="'.URL::index("&amp;file=delete&amp;id=$id&amp;what=picture").'" class="adm_menu" onclick="return confirm(\''.PIC_CONFIRM_DEL.'\');">'.DEL_PIC.'</a></div>';*/
...
function html_img_nav_menu()
{
	...
	if ($pos > 0) {
		$prev = $pos - 1;
		$prev_tgt = URL::index("&amp;file=displayimage$meta_link$cat_link&amp;pos=$prev");
	...
	if ($pos < ($pic_count -1)) {
		$next = $pos + 1;
		$next_tgt = URL::index("&amp;file=displayimage$meta_link$cat_link&amp;pos=$next");
	...
	if ((USER_CAN_SEND_ECARDS) && (USER_ID or $CONFIG['allow_anon_fullsize'] or USER_IS_ADMIN)) {
		$ecard_tgt = URL::index("&amp;file=ecard$meta_link$cat_link&amp;pid=$pid&amp;pos=$pos");
		$ecard_title = ECARD_TITLE;
	} else {
		$ecard_tgt = "javascript:alert('" . addcslashes(ECARD_DISABLED, "'") . "');";
		$ecard_title = ECARD_DISABLED;
	}
	$thumb_tgt = URL::index("&amp;file=thumbnails$meta_link$cat_link&amp;page=$page"); //$cat_link&page=$page
	...
	if (USER_ID or $CONFIG['allow_anon_fullsize'] or USER_IS_ADMIN ) {
		$slideshow_tgt = URL::index("&amp;file=displayimage$meta_link$cat_link&amp;pid=$pid&amp;slideshow=5000");
	...
	if (USER_ID > 1 or $CONFIG['allow_anon_fullsize'] or USER_IS_ADMIN) {
		...
		if (isset($image_size['reduced'])) {
			...
			if ($CONFIG['justso']) {
				...
				$winsizeX = $CURRENT_PIC_DATA['pwidth'] + 16;
				$winsizeY = $CURRENT_PIC_DATA['pheight'] + 16;
				$pic_html = '<a href="'.URL::index("&amp;file=displayimagepopup&amp;pid=$pid&amp;fullsize=1",true,true).'" target="' . uniqid(rand()) . "\" onclick=\"imgpop('".URL::index("&amp;file=displayimagepopup&amp;pid=$pid&amp;fullsize=1",true,true)."','" . uniqid(rand()) . "','resizable=yes,scrollbars=yes,width=$winsizeX,height=$winsizeY,left=0,top=0');return false\">"; //toolbar=yes,status=yes,
...
/*		'{RATE0}' => URL::index("&amp;file=ratepic&amp;pic=$pid&amp;rate=0"),
		'{RATE1}' => URL::index("&amp;file=ratepic&amp;pic=$pid&amp;rate=1"),
		'{RATE2}' => URL::index("&amp;file=ratepic&amp;pic=$pid&amp;rate=2"),
		'{RATE3}' => URL::index("&amp;file=ratepic&amp;pic=$pid&amp;rate=3"),
		'{RATE4}' => URL::index("&amp;file=ratepic&amp;pic=$pid&amp;rate=4"),
		'{RATE5}' => URL::index("&amp;file=ratepic&amp;pic=$pid&amp;rate=5"),
	);*/
	if (USER_ID or $CONFIG['allow_anon_fullsize'] or USER_IS_ADMIN) {
...
function html_comments($pid)
{
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		$params = array('{MSG_AUTHOR}' => $row['msg_author'],
			...
			'{DELETE_LINK}' => URL::index("&amp;file=delete"),
...
if ($meta == 'random' || ($meta == '' && !is_numeric($album)) || $pid > 0 || $pos < 0) {
	...
	if ($db->sql_numrows($result) == 0) {
		list($visibility) = $db->sql_ufetchrow("SELECT a.visibility FROM {$CONFIG['TABLE_PICTURES']} AS p INNER JOIN {$CONFIG['TABLE_ALBUMS']} AS a ON (p.aid = a.aid) AND p.pid=".$pid);
		if ($visibility ==2){
	...
	if ($pic_count == 0) {
		list($visibility) = $db->sql_ufetchrow("SELECT visibility FROM {$CONFIG['TABLE_ALBUMS']} WHERE aid=".$album);
		if ($visibility ==2){
			cpg_die(INFO, MEMBERS_ONLY, __FILE__, __LINE__);
		//works
		//} elseif ($visibility >= FIRST_USER_CAT){
