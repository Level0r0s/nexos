...
switch ($what) {
	...
	case 'comment':
		$msg_id = intval($_POST['msg_id']);
			...
			cpg_delete_msg(URL::index("&amp;file=delete"),$msg,'<input type="hidden" name="what" value="comment" /><input type="hidden" name="msg_id" value="'.$msg_id.'" />');		
	...
	case 'picture':
		...
		if (!isset($_POST['confirm'])) {
			$msg = PIC_CONFIRM_DEL;
			cpg_delete_msg(URL::index("&amp;file=delete"),$msg,'<input type="hidden" name="what" value="picture" /><input type="hidden" name="id" value="'.$pid.'" />');	   
		} else {
		...
		output_caption();
		echo "<tr><td colspan=\"6\" class=\"tablef\" align=\"center\">\n";
		echo "<div class=\"admin_menu_thumb\"><a href=\"" . URL::index("&amp;file=thumbnails&amp;album=$aid"). "\"  class=\"adm_menu\">".CONTINU."</a></div>\n";
		echo "</td></tr>\n";
		}
		endtable();
		pagefooter();
	...
	case 'album':
		...
		if(isset($_POST['cancel'])) {
			$redirect = GALLERY_ADMIN_MODE ? URL::index('&amp;file=albmgr&amp;cat='.$cat) : URL::index('&amp;cat='.$cat);
			//$redirect = URL::index("&file=albmgr&album=".$aid);
			URL::redirect($redirect);
		}
		if (!isset($_POST['confirm'])) {
			$msg = CONFIRM_DELETE1 . '<br />'.CONFIRM_DELETE2 . '<br />';
			cpg_delete_msg(URL::index("&amp;file=delete"),$msg,'<input type="hidden" name="what" value="album" /><input type="hidden" name="id" value="'.$aid.'" />');			
		} else {
		...
		echo "<tr><td colspan=\"6\" class=\"tablef\" align=\"center\">\n";
		echo "<div class=\"admin_menu_thumb\"><a href=\"" .URL::index("&amp;file=albmgr&amp;cat=$cat") ."\"  class=\"adm_menu\">".CONTINU."</a></div>\n";
		echo "</td></tr>";
		endtable();
		pagefooter();
		}
