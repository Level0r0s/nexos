 ...
 *
 *	$Id: modcp.php,v 9.19 2008/08/03 04:59:46 nanocaiordo Exp $
 *
 ***************************************************************************/
...
else if (isset($_POST['delete'])) { $mode = 'delete'; }
else if (isset($_POST['move']))   { $mode = 'move'; }
else if (isset($_POST['lock']))   { $mode = 'lock'; }
else if (isset($_POST['unlock'])) { $mode = 'unlock'; }
else { $mode = ''; }
...
//		  if (defined('BBAttach_mod')) {
		...
		if (!empty($topic_id)) {
		  ...
		  $l_redirect = sprintf($lang['Click_return_forum'], '<a href="'.URL::index($redirect_page).'">', '</a>');
		} else {
		  $redirect_page ="&file=modcp&".POST_FORUM_URL."=$forum_id";
		  $l_redirect = sprintf($lang['Click_return_modcp'], '<a href="'.URL::index($redirect_page).'">', '</a>');
		}
		URL::refresh(URL::index($redirect_page));
		...
		$template->assign_vars(array(
		  ...
		  'S_CONFIRM_ACTION' => URL::index("&amp;file=modcp"),
	...
	case 'move':
	  ...
	  if ($confirm) {
		...
		if ( !empty($topic_id) ) {
		  ...
		  $message .= sprintf($lang['Click_return_topic'], '<a href="'.URL::index($redirect_page).'">', '</a>');
		} else {
		  $redirect_page = "&file=modcp&".POST_FORUM_URL."=$forum_id";
		  $message .= sprintf($lang['Click_return_modcp'], '<a href="'.URL::index($redirect_page).'">', '</a>');
		}
		$message = $message.'<br \><br \>'.sprintf($lang['Click_return_forum'], '<a href="'.URL::index("&amp;file=viewforum&amp;".POST_FORUM_URL."=$old_forum_id").'">', '</a>');
		URL::refresh(URL::index($redirect_page));
		...
		$template->assign_vars(array(
		  ...
		  'S_MODCP_ACTION' => URL::index("&amp;file=modcp"),
		  'S_HIDDEN_FIELDS' => $hidden_fields)
		);
	...
	case 'lock':
	  ...
	  $message = $message.'<br \><br \>'.sprintf($lang['Click_return_forum'], '<a href="'.URL::index('&amp;file=viewforum&amp;'.POST_FORUM_URL."=$forum_id").'">', '</a>');
	  URL::refresh(URL::index($redirect_page));
	...
	case 'split':
	  ...
	  if ($post_id_sql != '') {
		...
		if ($row = $db->sql_fetchrow($result)) {
		  ...
		  $message = $lang['Topic_split'].'<br /><br />'.sprintf($lang['Click_return_topic'], '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id").'">', '</a>');
		  message_die(GENERAL_MESSAGE, $message);
		}
	  } else {
		...
		if( ( $total_posts = $db->sql_numrows($result) ) > 0 ) {
		  ...
		  $template->assign_vars(array(
			...
			'S_SPLIT_ACTION' => URL::index("&amp;file=modcp"),
	...
	case 'ip':
	  ...
	  $template->assign_vars(array(
		...
		'U_LOOKUP_IP' => URL::index("&amp;file=modcp&amp;mode=ip&amp;".POST_POST_URL."=$post_id&amp;".POST_TOPIC_URL."=$topic_id&amp;rdns=".$ip_this_post))
	  );
	  ...
	  if ( $row = $db->sql_fetchrow($result) ) {
		...
		do {
		  ...
		  $template->assign_block_vars('iprow', array(
			...
			'U_LOOKUP_IP' => URL::index("&amp;file=modcp&amp;mode=ip&amp;".POST_POST_URL."=$post_id&amp;".POST_TOPIC_URL."=$topic_id&amp;rdns=".decode_ip($row['poster_ip'])))
		  );
	  ...
	  if ( $row = $db->sql_fetchrow($result) ) {
		...
		do {
		  ...
		  $template->assign_block_vars('userrow', array(
			...
			'U_PROFILE' => URL::index("Your_Account&amp;profile=$id"),
	...
	default:
		...
		while ( $row = $db->sql_fetchrow($result) ) {
			...
			$u_view_topic = URL::index("&amp;file=modcp&amp;mode=split&amp;".POST_TOPIC_URL."=$topic_id");
