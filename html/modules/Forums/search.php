 ...
 *
 *	 $Id: search.php,v 9.17 2008/08/03 04:59:46 nanocaiordo Exp $
 *
 *
 ***************************************************************************/
...
if ( $mode == 'searchuser' ) {
	...
	if ( isset($_POST['search_username']) ) {
		if (false === strpos($_POST['search_username'], '*')) $_POST['search_username'] = '*'.$_POST['search_username'].'*';
	...
	if ( $search_results != '' ) {
		...
		for($i = 0; $i < count($searchset); $i++) {
			$forum_url = URL::index("&amp;file=viewforum&amp;" . POST_FORUM_URL . '=' . $searchset[$i]['forum_id']);
			$topic_url = URL::index("&amp;file=viewtopic&amp;" . POST_TOPIC_URL . '=' . $searchset[$i]['topic_id'] . (!empty($highlight_active) ? "&amp;highlight=$highlight_active" : ""));
			if (isset($searchset[$i]['post_id'])) {
				$post_url = URL::index("&amp;file=viewtopic&amp;" . POST_POST_URL . '=' . $searchset[$i]['post_id'] . (!empty($highlight_active) ? "&amp;highlight=$highlight_active" : "")) . '#' . $searchset[$i]['post_id'];
			} else {
				$post_url = '';
			}
			...
			else
			{
				...
				$topic_author = ( $searchset[$i]['user_id'] != ANONYMOUS ) ? '<a href="' . URL::index('Your_Account&amp;profile=' . $searchset[$i]['user_id']) . '">' : '';
				...
				$last_post_author = ( $searchset[$i]['id2'] == ANONYMOUS ) ? ( ($searchset[$i]['post_username2'] != '' ) ? $searchset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . URL::index('Your_Account&amp;profile=' . $searchset[$i]['id2']) . '">' . $searchset[$i]['user2'] . '</a>';

				$last_post_url = '<a href="' . URL::index("&amp;file=viewtopic&amp;"  . POST_POST_URL . '=' . $searchset[$i]['topic_last_post_id']) . '#' . $searchset[$i]['topic_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" /></a>';
