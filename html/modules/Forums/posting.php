 ...
 *
 *	 $Id: posting.php,v 9.24 2008/08/03 04:59:46 nanocaiordo Exp $
 *
 *
 ***************************************************************************/
...
if ( isset($_POST['cancel']) ) {
	...
	URL::redirect(URL::index('&file=viewforum&'.POST_FORUM_URL."=$forum_id"));
	} else {
...
if ( $result = $db->sql_query($sql) )
{
	...
	if ( $mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete' ) {
		...
		if ($post_info['poster_id'] != $userdata['user_id'] && !$is_auth['auth_mod']) {
			...
			$message .= '<br /><br />'.sprintf($lang['Click_return_topic'], '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id").'">', '</a>');
...
if (($delete || $poll_delete || $mode == 'delete') && !$confirm) {
	...
	$template->assign_vars(array(
		...
		'S_CONFIRM_ACTION' => URL::index("&amp;file=posting"),
	...
	if (!empty($_POST['vote_id'])) {
		...
		$message .=	 '<br /><br />'.sprintf($lang['Click_view_message'], '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id").'">', '</a>');
		message_die(GENERAL_MESSAGE, $message);
	} else {
		URL::redirect(URL::index("&file=viewtopic&".POST_TOPIC_URL."=$topic_id", true));
	}
...
$template->assign_vars(array(
	...
	'BBCODE_STATUS' => sprintf($bbcode_status, '<a href="'.URL::index("&amp;file=faq&amp;mode=bbcode").'" target="_phpbbcode">', '</a>'),
	...
	'U_VIEWTOPIC' => ( $mode == 'reply' ) ? URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;postorder=desc") : '',
	'U_REVIEW_TOPIC' => ( $mode == 'reply' ) ? URL::index("&amp;file=posting&amp;mode=topicreview&amp;".POST_TOPIC_URL."=$topic_id&amp;popup=1") : '',
