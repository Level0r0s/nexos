...
if ($submit) {
	...
	if ($confirm) {
		...
		// send end message
		URL::refresh(URL::index("&file=viewtopic&".POST_TOPIC_URL."=$to_topic_id"));
		message_die(GENERAL_MESSAGE, $lang['Merge_topic_done'].'<br /><br />'.sprintf($lang['Click_return_topic'], '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$to_topic_id").'" class="gen">', '</a>')	.'<br /><br />'.sprintf($lang['Click_return_index'], '<a href="'.URL::index().'" class="gen">', '</a>'));
		exit;
	}
	else
	{
		...
		$template->assign_vars(array(
			...
			'L_YES'			   => $lang['Yes'],
			'L_NO'			   => $lang['No'],
			'S_CONFIRM_ACTION' => URL::index("&amp;file=merge"),
			'S_HIDDEN_FIELDS'  => $s_hidden_fields,
			)
		);
		// footer
