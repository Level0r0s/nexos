...
if ( isset($_GET['view']) && empty($_GET[POST_POST_URL]) ) {
	if ( $_GET['view'] == 'newest' ) {
		if (is_user()) {
			...
			$post_id = $row['post_id'];
			URL::redirect(URL::index("&file=viewtopic&".POST_POST_URL."=$post_id")."#$post_id");
		}
		URL::redirect(URL::index("&file=viewtopic&".POST_TOPIC_URL."=$topic_id"));
		...
		$sql = "SELECT t.topic_id FROM ".TOPICS_TABLE." t, ".TOPICS_TABLE." t2
			...
			ORDER BY t.topic_last_post_id $sql_ordering LIMIT 1";
		$result = $db->sql_query($sql);
		if ( $row = $db->sql_fetchrow($result, SQL_ASSOC) ) {
...
$sql = "SELECT t.topic_id, t.topic_title, t.topic_status, t.topic_replies, t.topic_time, t.topic_type, t.topic_vote, t.topic_last_post_id, f.forum_name, f.forum_desc, f.forum_status, f.forum_id, f.parent_id, f.auth_view, f.auth_read, f.auth_post, f.auth_reply, f.auth_edit, f.auth_delete, f.auth_sticky, f.auth_announce, f.auth_pollcreate, f.auth_vote, f.auth_attachments, f.auth_download, t.topic_attachment, c.cat_title ".$count_sql."
...
if (is_user()) {
	...
	if ( $row = $db->sql_fetchrow($result) ) {
		if ( isset($_GET['unwatch']) ) {
			...
			$message = $lang['No_longer_watching'].'<br /><br />'.sprintf($lang['Click_return_topic'], '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;start=$start").'">', '</a>');
			message_die(GENERAL_MESSAGE, $message);
		} else {
...
if ($resync) {
	require_once('includes/phpBB/functions_admin.php');
	sync('topic', $topic_id);
	$result = $db->sql_query('SELECT COUNT(post_id) AS total FROM '.POSTS_TABLE.'
	WHERE topic_id = '.$topic_id);
...
$view_next_topic_url = URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;view=next");
...
if (is_user()) {
	...
	$CPG_SESS[$module_name]['track_topics'][$topic_id] = time();
}
...
if ( $is_auth['auth_mod'] ) {
	...
	$topic_mod .= '<a href="'.URL::index("&amp;file=modcp&amp;".POST_TOPIC_URL."=$topic_id&amp;mode=delete").'"><img src="'.$images['topic_mod_delete'].'" alt="'.$lang['Delete_topic'].'" title="'.$lang['Delete_topic'].'" style="border:0;" /></a>&nbsp;';
	$topic_mod .= '<a href="'.URL::index("&amp;file=modcp&amp;".POST_TOPIC_URL."=$topic_id&amp;mode=move"). '"><img src="'.$images['topic_mod_move'].'" alt="'.$lang['Move_topic'].'" title="'.$lang['Move_topic'].'" style="border:0;" /></a>&nbsp;';
	$topic_mod .= ( $forum_topic_data['topic_status'] == TOPIC_UNLOCKED ) ? '<a href="'.URL::index("&amp;file=modcp&amp;".POST_TOPIC_URL."=$topic_id&amp;mode=lock").'"><img src="'.$images['topic_mod_lock'].'" alt="'.$lang['Lock_topic'].'" title="'.$lang['Lock_topic'].'" style="border:0;" /></a>&nbsp;' : '<a href="'.URL::index("&amp;file=modcp&amp;".POST_TOPIC_URL."=$topic_id&amp;mode=unlock").'"><img src="'.$images['topic_mod_unlock'].'" alt="'.$lang['Unlock_topic'].'" title="'.$lang['Unlock_topic'].'" style="border:0;" /></a>&nbsp;';
	$topic_mod .= '<a href="'.URL::index("&amp;file=modcp&amp;".POST_TOPIC_URL."=$topic_id&amp;mode=split").'"><img src="'.$images['topic_mod_split'].'" alt="'.$lang['Split_topic'].'" title="'.$lang['Split_topic'].'" style="border:0;" /></a>&nbsp;';
//-- mod : merge -----------------------------------------------------------------------------------
	$topic_mod .= '<a href="'.URL::index("&amp;file=merge&amp;".POST_TOPIC_URL.'='.$topic_id).'"><img src="'.$images['topic_mod_merge'].'" alt="'.$lang['Merge_topics'].'" title="'.$lang['Merge_topics'].'" style="border:0;" /></a>&nbsp;';
//-- fin mod : merge -------------------------------------------------------------------------------
}
...
if ( $can_watch_topic ) {
	if ( $is_watching_topic ) {
		$s_watching_topic = '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;unwatch=topic&amp;start=$start").'">'.$lang['Stop_watching_topic'].'</a>';
		$s_watching_topic_img = ( isset($images['Topic_un_watch']) ) ? '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;unwatch=topic&amp;start=$start").'"><img src="'.$images['Topic_un_watch'].'" alt="'.$lang['Stop_watching_topic'].'" title="'.$lang['Stop_watching_topic'].'" style="border:0;" /></a>' : '';
	} else {
		$s_watching_topic = '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;watch=topic&amp;start=$start").'">'.$lang['Start_watching_topic'].'</a>';
		$s_watching_topic_img = ( isset($images['Topic_watch']) ) ? '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;watch=topic&amp;start=$start").'"><img src="'.$images['Topic_watch'].'" alt="'.$lang['Stop_watching_topic'].'" title="'.$lang['Start_watching_topic'].'" style="border:0;" /></a>' : '';
	}
}
...
if ($pagination != '' && !empty($pagination_printertopic)) {
	$pagination .= ' &nbsp;<a href="'.URL::index('&amp;file=viewtopic&amp;?'. $pagination_printertopic. POST_TOPIC_URL."=$topic_id&amp;postdays=$post_days&amp;postorder=$post_order".$pagination_highlight.'&amp;start=0&amp;finish_rel=-10000').'" title="	 :| |:	">:|&nbsp;|:</a>';
}
...
$template->assign_vars(array(
		...
		'S_POST_DAYS_ACTION' => URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;start=$start"),
		'S_AUTH_LIST' => $s_auth_can,
		'S_TOPIC_ADMIN' => $topic_mod,
		'S_WATCH_TOPIC' => $s_watching_topic,
		'S_WATCH_TOPIC_IMG' => $s_watching_topic_img,

		'U_VIEW_TOPIC' => URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;start=$start&amp;postdays=$post_days&amp;postorder=$post_order&amp;highlight=$highlight"),
	...
	if ( $vote_info ) {
		...
		else
		{
			...
			$template->assign_vars(array(
				...
				'U_VIEW_RESULTS' => URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;postdays=$post_days&amp;postorder=$post_order&amp;vote=viewresult"))
			);
		...
		$template->assign_vars(array(
			...
			'S_POLL_ACTION' => URL::index("&amp;file=posting&amp;mode=vote&amp;".POST_TOPIC_URL."=$topic_id"))
		);
	}
...
for ($i = 0; $i < $total_posts; $i++) {
	...
	$mini_post_url = URL::index("&amp;file=viewtopic&amp;".POST_POST_URL.'='.$postrow[$i]['post_id']).'#'.$postrow[$i]['post_id'];
	...
	if ($poster_id != ANONYMOUS) {
		$profile = array(
			...
			'URL' => URL::index("Your_Account&amp;profile=$poster_id"),
			'TARGET' => false
		);
		if ($old_theme) {
		...
		if (is_user() && is_active("Private_Messages")) {
			$pm = array(
				...
				'URL' => URL::index("Private_Messages&amp;mode=post&amp;".POST_USERS_URL."=$poster_id"),
				'TARGET' => false
			);
			if ($old_theme) {
		...
		if (!empty($postrow[$i]['user_viewemail']) || $is_auth['auth_mod']) {
			$email = array(
				...
				'URL' => ($board_config['board_email_form']) ? URL::index("&amp;file=profile&amp;mode=email&amp;".POST_USERS_URL.'='.$poster_id) : 'mailto:'.$postrow[$i]['user_email'],
				'TARGET' => false
			);
			if ($old_theme) {
		...
		if ($ugall) {
			...
			if (!isset($ugalleries[$user_gallery])) {
				$ugall_result = $db->sql_ufetchrow("SELECT COUNT(*) FROM ".$ugall."pictures AS p, ".$ugall."albums AS a WHERE a.aid=p.aid AND a.category=$user_gallery");
			...
			if ($ugalleries[$user_gallery]){
				$gal = array(
					...
					'URL' => URL::index($ugalldir."&amp;cat=".$user_gallery),
					'TARGET' => false
				);
				if ($old_theme) {
	...
	$temp_url = URL::index("&amp;file=posting&amp;mode=quote&amp;".POST_POST_URL."=".$postrow[$i]['post_id']);
	...
	$temp_url = URL::index("&amp;file=search&amp;search_author=".urlencode($postrow[$i]['username'])."&amp;showresults=posts");
	...
	if ( ( $userdata['user_id'] == $poster_id && $is_auth['auth_edit'] ) || $is_auth['auth_mod'] ) {
		$temp_url = URL::index("&amp;file=posting&amp;mode=editpost&amp;".POST_POST_URL."=".$postrow[$i]['post_id']);
	...
	if ( $is_auth['auth_mod'] ) {
		$temp_url = URL::index("&amp;file=modcp&amp;mode=ip&amp;".POST_POST_URL."=".$postrow[$i]['post_id']."&amp;".POST_TOPIC_URL."=".$topic_id);
		$ip_img = '<a href="'.$temp_url.'"><img src="'.$images['icon_ip'].'" alt="'.$lang['View_IP'].'" title="'.$lang['View_IP'].'" style="border:0;" /></a>';
		$ip = '<a href="'.$temp_url.'">'.$lang['View_IP'].'</a>';
		$temp_url = URL::index("&amp;file=posting&amp;mode=delete&amp;".POST_POST_URL."=".$postrow[$i]['post_id']);
