...
$moderators = !empty($moderators) ? implode(', ', $moderators) : $lang['None']; // Moderators: None
...
//			'SUBFORUMS'		  => $subforums_list,
			'L_SUBFORUM_STR'	 => $forum_data[$j]['subforums_lang'],
//			'L_SUBFORUM_STR'	 => $l_subforums,
			'L_MODERATOR_STR' => $forum_data[$j]['l_moderators'],
			'L_FORUM_FOLDER_ALT' => $forum_data[$j]['folder_alt'],
			'BC_DELIM'		  => _BC_DELIM,
			'U_LAST_POSTER'	  => ($forum_data[$j]['user_id'] > ANONYMOUS) ? URL::index("Your_Account&amp;profile=".$forum_data[$j]['user_id']) : '',
			'U_LAST_POST'	  => ($forum_data[$j]['forum_last_post_id']) ? URL::index("&amp;file=viewtopic&amp;" .POST_POST_URL.'='.$forum_data[$j]['forum_last_post_id']).'#'.$forum_data[$j]['forum_last_post_id'] : '',
...
if ($forum_row['parent_id'] != 0) {
	$is_subforum = true;
	$parent_id = $forum_row['parent_id'];
	$parents = array();
	while ($parent_id != 0) {
		list ($parent_name, $parent_id, $parent_forum_id) = $db->sql_ufetchrow("SELECT forum_name AS parent_name, parent_id, forum_id FROM ".FORUMS_TABLE." 
		WHERE forum_id = $parent_id", SQL_NUM);
		$parents[] = '<a href="'.URL::index('&amp;file=viewforum&amp;f='.$parent_forum_id).'">'.$parent_name.'</a>';
	}
...
$template->assign_vars(array(
	...
	'S_POST_DAYS_ACTION' => URL::index("&amp;file=viewforum&amp;".POST_FORUM_URL."=".$forum_id."&amp;start=$start"),

	'U_MARK_READ' => URL::index("&amp;file=viewforum&amp;".POST_FORUM_URL."=$forum_id&amp;mark=topics"),
	'U_POST_NEW_TOPIC' => URL::index("&amp;file=posting&amp;mode=newtopic&amp;".POST_FORUM_URL."=$forum_id"),
	'U_VIEW_FORUM' => URL::index("&amp;file=viewforum&amp;".POST_FORUM_URL ."=$forum_id")
	)
);
...
// TopicIcon_mod end
				...
				if ($topic_rowset[$i]['post_time'] > $userdata['user_lastvisit']) {
					if (!empty($tracking_topics) || !empty($tracking_forums) || isset($CPG_SESS[$module_name]['track_all'])) {
						...
						if ($unread_topics) {
							...
							$newest_post_img = '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=$topic_id&amp;view=newest").'"><img src="'.$images['icon_newest_reply'].'" alt="'.$lang['View_newest_post'].'" title="'.$lang['View_newest_post'].'" /></a> ';
						} else {
							$folder_image = $folder;
		...
		if (($replies+1) > $board_config['posts_per_page']) {
			...
			for ($j = 0; $j < $replies + 1; $j += $board_config['posts_per_page']) {
				$goto_page .= '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_TOPIC_URL."=".$topic_id."&amp;start=$j").'">'.$times.'</a>';
...
// BBAttach_mod
			...
			'LAST_POST_AUTHOR' => ($topic_rowset[$i]['id2'] == ANONYMOUS) ? (($topic_rowset[$i]['post_username2'] != '') ? $topic_rowset[$i]['post_username2'].' ' : $lang['Guest'].' ' ) : '<a href="'.URL::index("Your_Account&amp;profile=".$topic_rowset[$i]['id2']).'">'.$topic_rowset[$i]['user2'].'</a>',
			'LAST_POST_IMG' => '<a href="'.URL::index("&amp;file=viewtopic&amp;".POST_POST_URL.'='.$topic_rowset[$i]['topic_last_post_id']).'#'.$topic_rowset[$i]['topic_last_post_id'].'"><img src="'.$images['icon_latest_reply'].'" alt="'.$lang['View_latest_post'].'" title="'.$lang['View_latest_post'].'" /></a>',
