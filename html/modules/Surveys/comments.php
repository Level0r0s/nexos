...
function modtwo($tid, $score, $reason) {
	global $MAIN_CFG;
	$return = '';
	if((is_admin() && $MAIN_CFG['global']['moderate'] > 0) || ($MAIN_CFG['global']['moderate'] == 2 && is_user())) {
...
function navbar($poll_id, $title, $thold, $mode, $order) {
	global $MAIN_CFG, $prefix, $db, $mainindex, $cpgtpl;
	...
	$cpgtpl->assign_vars(array(
		...
		'S_POSTCOMM' => ($MAIN_CFG['global']['anonpost'] || is_user()) ? _REPLYMAIN : '',
		...
		'S_REFRESH'	 => _REFRESH,
		'U_ARTICLE'	 => URL::index('&amp;op=results&amp;pollid='.$poll_id),
		'U_TOPREPLY' => URL::index('&amp;reply=0&amp;pollid='.$poll_id),
		'U_USER'	 => (is_user() ? URL::index('Your_Account&amp;op=editcomm') : URL::index("Your_Account"))
	));
}

function DisplayKids($tid, $mode, $order=0, $thold=0, $level=0, $dummy=0) {
	global $userinfo, $MAIN_CFG, $prefix, $db, $cpgtpl;
	$comments = 0;
		...
		while ($row = $db->sql_fetchrow($result)) {
			if($row['score'] >= $thold) {
				...
				$cpgtpl->assign_block_vars('comment', array(
					...
					'U_USERINFO'   => (isset($usrdata) ? URL::index("Your_Account&amp;profile=$usrdata[user_id]") : ''),
					'S_USERPM'	   => _SENDAMSG,
					'U_USERPM'	   => ((isset($usrdata) && is_user() && is_active('Private_Messages')) ? URL::index("Private_Messages&amp;mode=post&amp;u=$usrdata[user_id]") : ''),
					'U_COMMENTEXT' => (isset($commentext) ? URL::index("&amp;comment=$tid&amp;pollid=$poll_id") : ''),
					'U_DELETE'	   => is_admin() ? URL::admin("comments&amp;polldel=$tid") : '',
					'U_PARENT'	   => ($pid != 0) ? URL::index("&amp;comment=0&amp;pollid=$poll_id&amp;pid=$parentid") : '',
					'U_REPLY'	   => ($MAIN_CFG['global']['anonpost'] || is_user()) ? URL::index("&amp;reply=$tid&amp;pollid=$poll_id") : '',
				));
				$comments++;
		...
		while ($row = $db->sql_fetchrow($result)) {
			if($row['score'] >= $thold) {
				...
				$cpgtpl->assign_block_vars('comment', array(
					...
					'IS_FIRST_END' => false,
					'IS_LIST'	 => true,
					'U_READCOMM' => URL::index("&amp;comment=$row[tid]&amp;pollid=$row[poll_id]&amp;pid=$row[pid]")."#$row[tid]",
...
function DisplayComments($poll_id, $title, $pid=0, $tid=0) {
	global $userinfo, $MAIN_CFG, $prefix, $db, $bgcolor1, $bgcolor2, $bgcolor3, $cpgtpl, $CPG_SESS;
...
function singlecomment($tid, $poll_id) {
	global $bgcolor1, $bgcolor2, $textcolor2, $MAIN_CFG, $prefix, $db, $cpgtpl;
	require_once('header.php');
	...
	if ($MAIN_CFG['global']['anonpost'] || is_admin() || is_user()) {
		echo '<font class="content"> [ <a href="'.URL::index("&amp;reply=$tid&amp;pollid=$poll_id").'">'._REPLY.'</a> | <a href="'.URL::index('&amp;op=results&amp;pollid='.$poll_id).'">'._ROOT.'</a>';
	}
...
function replyform($poll_id, $pid, $subject='', $comment='') {
	global $userinfo;
	if (false === stripos($subject, 'Re:')) $subject = 'Re: '.substr($subject,0,81);
	if (is_user()) {
		$user = '<a href="'.URL::index('Your_Account').'">'.$userinfo['username'].'</a>';
	} else {
		$user = _ANONYMOUS.' [ <a href="'.URL::index('Your_Account').'">'._NEWUSER.'</a> ]';
	}
	OpenTable();
	echo '<form action="'.URL::index().'" method="post" name="postcomment" enctype="multipart/form-data" accept-charset="utf-8"><div>
...
function reply($poll_id) {
	...
	require_once('header.php');
	global $db, $prefix, $userinfo, $bgcolor1, $bgcolor2, $bgcolor3, $MAIN_CFG;
...
function replyPost($poll_id) {
	...
	if (!$MAIN_CFG['global']['anonpost'] && !is_user()) { cpg_error(_NOANONCOMMENTS); }
	...
	if ($fake) {
		$db->sql_query("INSERT INTO ".$prefix."_pollcomments (tid, pid, poll_id, date, name, email, url, host_name, subject, comment, score, reason) VALUES (DEFAULT, '$pid', '$poll_id', ".time().", '$name', '$email', '$url', '$ip', '$subject', '$comment', '$score', '0')");
	} else {
	...
	URL::redirect(URL::index('&amp;op=results&amp;pollid='.$poll_id));
}
	...
	if ((is_admin() && $MAIN_CFG['global']['moderate'] > 0) || ($MAIN_CFG['global']['moderate'] == 2 && is_user())) {
		...
		if (false !== strpos($key, 'dkn')) {
			...
			if($val[1] != 0 && $val[0] > -1 && $val[0] < 5) {
				...
				$db->sql_query($q.' WHERE tid='.intval(str_replace('dkn', '', $key)));
			}
		}
		}
	}
