...
if(!isset($_POST['pollid']) && !isset($_GET['pollid'])) {
	...
	$querylang = '';
	if ($MAIN_CFG['global']['multilingual']) { $querylang = "AND (planguage='$currentlang' OR planguage='')"; }
	...
	while(list($id, $title, $voters) = $db->sql_fetchrow($result)) {
		if (is_admin()) { $editing = ' - <a href="'.URL::admin('Surveys&amp;mode=edit&amp;id='.$id).'">'._EDIT.'</a>'; }
		echo "<b>&#8226;</b>&nbsp;<a href=\"".URL::index("&amp;pollid=$id")."\">$title</a> ";
		echo "(<a href=\"".URL::index("&amp;op=results&amp;pollid=$id")."\">"._RESULTS."</a> - $voters "._LVOTES."$editing)<br />\n";
	}
	...
	if ($db->sql_numrows($result)) {
		...
		while(list($id, $title, $voters) = $db->sql_fetchrow($result)) {
			...
			echo '(<a href="'.URL::index('&amp;op=results&amp;pollid='.$id).'">'._RESULTS."</a> - $voters "._LVOTES."$editing)<br />\n"
			._ATTACHEDTOARTICLE.' <a href="'.URL::index('News&amp;file=article&amp;sid='.$sid)."\">$title</a><br /><br />\n";
		}
		echo '</td></tr></table>';
	}
	$db->sql_freeresult($result);
...
if (isset($_POST['vote_id'])) {
	...
	if (strlen($forwarder<5)) $forwarder=URL::index('&op=results&pollid='.$poll_id);
...
function pollMain($poll_id) {
	...
	$boxContent .= "<input type=\"hidden\" name=\"forwarder\" value=\"".URL::index("&amp;op=results&amp;pollid=$poll_id")."\" />";
	...
	$boxContent .= '<br /><font class="content"><a href="'.URL::index('&amp;op=results&amp;pollid='.$poll_id).'"><b>'._RESULTS.'</b></a> <b>::</b> <a href="'.URL::index().'"><b>'._POLLS.'</b></a><br />';
...
function pollLatest() {
	global $prefix, $MAIN_CFG, $currentlang, $db;
	$querylang = ($MAIN_CFG['global']['multilingual']) ? "AND planguage='$currentlang' OR planguage=''" : '';
	$poll_id = $db->sql_ufetchrow('SELECT poll_id FROM '.$prefix."_poll_desc 
		WHERE artid=0 $querylang 
		ORDER BY poll_id DESC", SQL_NUM);
	return $poll_id[0];
}

function pollVoted($poll_id) {
	global $prefix, $db, $userinfo;
	$result = $db->sql_query("SELECT anonymous FROM ".$prefix."_poll_desc WHERE poll_id='$poll_id' LIMIT 1");
...
function pollResults($poll_id) {
	...
	echo '[ <a href="'.URL::index("&amp;pollid=$poll_id")."\">"._VOTING."</a> | "
		.'<a href="'.URL::index().'">'._OTHERPOLLS."</a> ] $article </font></center>";
	if (can_admin('surveys')) { echo '<br /><center>[ <a href="'.URL::admin('Surveys&amp;mode=add').'">'._ADD.'</a> | <a href="'.URL::admin("Surveys&amp;mode=edit&amp;id=$poll_id").'">'._EDIT.'</a> ]</center>'; }
	return $holdtitle[0];
}
