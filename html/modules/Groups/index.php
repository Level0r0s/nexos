...
$groupurl = URL::index("&amp;g=$group_id", true, true);
...
//shows the given groups members to moderator allows moderator to add or approve members
{
			...
			if (isset($_POST['add'])) {
				...
				if (!$db->sql_numrows($result)) {
					...
					$emailer->assign_vars(array(
						...
						'U_GROUPCP' => URL::index('&'.POST_GROUPS_URL."=$group_id", true, true)
						)
					);
					$emailer->send();
					$emailer->reset();
	...
	$template->assign_vars(array(
		...
		'U_MOD_VIEWPROFILE' => URL::index('Your_Account&amp;profile='.$group_moderator['user_id']),
		...
		'S_GROUPCP_ACTION' => URL::index("&g=$group_id"))
	);
	...
	for ($i = $start; $i < min($board_config['topics_per_page'] + $start, $members_count); $i++) {
		...
		if ( $group_info['group_type'] != GROUP_HIDDEN || $is_group_member || $is_moderator ) {
			...
			$template->assign_block_vars('member_row', array(
				...
				'U_VIEWPROFILE' => URL::index('Your_Account&amp;profile='.$group_members[$i]['user_id']))
			);
	...
	if ($is_moderator && $modgroup_pending_count) {
		...
		for($i = 0; $i < $modgroup_pending_count; $i++) {
			...
			$template->assign_block_vars('pending_members_row', array(
				...
				'U_VIEWPROFILE' => URL::index('Your_Account&amp;profile='.$modgroup_pending_list[$i]['user_id']))
			);
		}
