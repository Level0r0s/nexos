...
/* Top 10 read stories */
$result = $db->sql_query("SELECT sid, title, counter FROM ".$prefix."_stories $queryalang ORDER BY counter DESC LIMIT $top");
if ($db->sql_numrows($result) > 0) {
	...
	while (list($sid, $title, $counter) = $db->sql_fetchrow($result)) {
		if ($counter > 0) {
			echo '<b>&#8226;</b>&nbsp;'.$rank.': <a href="'.URL::index("News&amp;file=article&amp;sid=$sid").'">'.$title.'</a> - ('.$counter.' '._READS.')<br />';
			$rank++;
		}
	}
	echo '</span></div><hr />';
...
$result = $db->sql_query("SELECT sid, title, ratings FROM ".$prefix."_stories $querya1lang score!='0' ORDER BY ratings DESC LIMIT $top");
if ($db->sql_numrows($result) > 0) {
	echo '<div style="padding:10px;">
	...
	while (list($sid, $title, $ratings) = $db->sql_fetchrow($result)) {
		if ($ratings > 0) {
			echo '<b>&#8226;</b>&nbsp;'.$rank.': <a href="'.URL::index("News&amp;file=article&amp;sid=$sid").'">'.$title.'</a> - ('.$ratings.' '._LVOTES.')<br />';
			$rank++;
		}
	}
	echo '</span></div><hr />';
...
$result = $db->sql_query("SELECT sid, title, score, ratings FROM ".$prefix."_stories $querya1lang score!='0' ORDER BY ratings+score DESC LIMIT $top");
if ($db->sql_numrows($result) > 0) {
	echo '<div style="padding:10px;">
	...
	while (list($sid, $title, $score, $ratings) = $db->sql_fetchrow($result)) {
		if ($score > 0) {
			...
			echo '<b>&#8226;</b>&nbsp;'.$rank.': <a href="'.URL::index("News&amp;file=article&amp;sid=$sid").'">'.$title.'</a> - ('.$rate.' '._POINTS.')<br />';
			$rank++;
		}
	}
	echo '</span></div><hr />';
...
if ($MAIN_CFG['global']['articlecomm']) {
	$result = $db->sql_query("SELECT sid, title, comments FROM ".$prefix."_stories $queryalang ORDER BY comments DESC LIMIT $top");
	if ($db->sql_numrows($result) > 0) {
		echo '<div style="padding:10px;">
		...
		while (list($sid, $title, $comments) = $db->sql_fetchrow($result)) {
			if ($comments > 0) {
				echo '<b>&#8226;</b>&nbsp;'.$rank.': <a href="'.URL::index("News&amp;file=article&amp;sid=$sid").'">'.$title.'</a> - ('.$comments.' '._COMMENTS.')<br />';
				$rank++;
			}
		}
		echo '</span></div><hr />';
...
/* Top 10 categories */
$result = $db->sql_query("SELECT catid, title, counter FROM ".$prefix."_stories_cat ORDER BY counter DESC LIMIT $top");
if ($db->sql_numrows($result) > 0) {
	echo '<div style="padding:10px;">
	...
	while (list($catid, $title, $counter) = $db->sql_fetchrow($result)) {
		if ($counter > 0) {
			echo '<b>&#8226;</b>&nbsp;'.$rank.': <a href="'.URL::index("News&amp;catid=$catid").'">'.$title.'</a> - ('.$counter.' '._HITS.')<br />';
			$rank++;
		}
	}
	echo '</span></div><hr />';
...
if (is_active('Sections')) {
	$result = $db->sql_query("SELECT artid, secid, title, content, counter FROM ".$prefix."_seccont $queryslang ORDER BY counter DESC LIMIT $top");
	if ($db->sql_numrows($result) > 0) {
		echo '<div style="padding:10px;">
		...
		while (list($artid, $secid, $title, $content, $counter) = $db->sql_fetchrow($result)) {
			echo '<b>&#8226;</b>&nbsp;'.$rank.': <a href="'.URL::index("Sections&amp;op=viewarticle&amp;artid=$artid").'">'.$title.'</a> - ('.$counter.' '._READS.')<br />';
			$rank++;
		}
		echo '</span></div><hr />';
	}
...
$result = $db->sql_query("SELECT user_id, username, counter FROM ".$user_prefix."_users WHERE counter > '0' ORDER BY counter DESC LIMIT $top");
if ($db->sql_numrows($result) > 0) {
	echo '<div style="padding:10px;">
	...
	while ($row = $db->sql_fetchrow($result)) {
		if ($row['counter'] > 0) {
			echo '<b>&#8226;</b>&nbsp;'.$rank.': <a href="'.URL::index("Your_Account&amp;profile=$row[user_id]").'">'.$row['username'].'</a> - ('.$row['counter'].' '._NEWSSENT.')<br />';
			$rank++;
		}
	}
	echo '</span></div><hr />';
...
if (is_active('Surveys')) {
	...
	<span class="option"><strong>'.$top.' '._VOTEDPOLLS.'</strong></span><br /><br /><span class="content">';
		$rank = 1;
		while ($row = $db->sql_fetchrow($result)) {
			echo '<b>&#8226;</b>&nbsp;'.$rank.': <a href="'.URL::index("Surveys&amp;pollid=$row[poll_id]").'">'.$row['poll_title'].'</a> - ('.$row['sum'].' '._LVOTES.')<br />';
			$rank++;
		}
		echo '</span></div><hr />';
	}
...
if (is_active('Reviews')) {
	$result = $db->sql_query("SELECT id, title, hits FROM ".$prefix."_reviews $queryrlang ORDER BY hits DESC LIMIT $top");
	if ($db->sql_numrows($result) > 0) {
		echo '<div style="padding:10px;">
		...
		while (list($id, $title, $hits) = $db->sql_fetchrow($result)) {
			if ($hits > 0) {
				echo '<b>&#8226;</b>&nbsp;'.$rank.': <a href="'.URL::index("Reviews&amp;op=showcontent&amp;id=$id").'">'.$title.'</a> - ('.$hits.' '._READS.')<br />';
				$rank++;
			}
		}
		echo '</span></div><hr />';
...
if (is_active('Content')) {
	$result = $db->sql_query("SELECT pid, title, counter FROM ".$prefix."_pages WHERE active='1' ORDER BY counter DESC LIMIT {$MAIN_CFG['global']['top']}");
	if ($db->sql_numrows($result) > 0) {
		echo '<div style="padding:10px;">
		...
		while (list($pid, $title, $counter) = $db->sql_fetchrow($result)) {
			if ($counter > 0) {
				echo '<b>&#8226;</b>&nbsp;'.$rank.': <a href="'.URL::index("Content&amp;pa=showpage&amp;pid=$pid").'">'.$title.'</a> ('.$counter.' '._READS.')<br />';
				$rank++;
			}
		}
		echo '</span></div>';
