...
$template->assign_vars(array(
	...
	'S_MODE_ACTION' => $home ? '' : URL::index('Members_List')
));

switch ($mode) {
	case 'username': $order_by = 'username'; break;
	case 'location': $order_by = 'user_from'; break;
	case 'posts':    $order_by = 'user_posts'; break;
	case 'website':  $order_by = 'user_website'; break;
	default:         $order_by = 'user_id'; break;
}
$order_by .= " $sort_order";
...
if ($row = $db->sql_fetchrow($result)) {
	...
	do {
		...
		$user_id = $row['user_id'];
		if (!preg_match('#^(http[s]?\:\/\/)?([a-z0-9\-\.]+)?[a-z0-9\-]+\.[a-z]{2,4}$#i', $row['user_website'])) {
			$row['user_website'] = '';
		};
		if (($row['user_website'] != '' ) && (substr($row['user_website'],0, 3) != 'http://')) {
		...
		$temp_url = URL::index('Your_Account&amp;profile='.$user_id);
		...
		if (is_user() && is_active('Private_Messages')) {
			$temp_url = URL::index('Private_Messages&amp;mode=post&amp;u='.$user_id);
		...
		$temp_url = URL::index('Forums&amp;file=search&amp;search_author='.urlencode($username).'&amp;showresults=posts');
		...
		$template->assign_block_vars('memberrow', array(
			...
			'YIM_IMG' => $yim_img,
			'YIM' => $yim,
			'U_VIEWPROFILE' => URL::index('Your_Account&amp;profile='.$user_id)
		));
...
AND user_level > 0";
list($total_members) = $db->u_fetch_row($sql,SQL_NUM);
...
function members_pagination($base_url, $num_items, $per_page, $start_item, $add_prevnext_text = TRUE)
{
	...
	if ( $total_pages > 10 ) {
		...
		for($i = 1; $i < $init_page_max + 1; $i++) {
			$page_string .= ( $i == $on_page ) ? '<b>'.$i.'</b>' : '<a href="'.URL::index($base_url.'&amp;start='.( ( $i - 1 ) * $per_page ) ).'">'.$i.'</a>';
	...
	if ( $add_prevnext_text ) {
		if ( $on_page > 1 ) {
			$page_string = ' <a href="'.URL::index($base_url.'&amp;start='.( ( $on_page - 2 ) * $per_page ) ).'">'.$lang['Previous'].'</a>&nbsp;&nbsp;'.$page_string;
		}
		if ( $on_page < $total_pages ) {
			$page_string .= '&nbsp;&nbsp;<a href="'.URL::index($base_url.'&amp;start='.( $on_page * $per_page ) ).'">'.$lang['Next'].'</a>';
		}
	}
