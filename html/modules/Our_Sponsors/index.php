...
function display_all_banners($row) {
	...
	if ($textban) {
		echo '<table style="margin:auto; width:'.$text_width.'px; border:0; height:'.$text_height.'px; background:#'.$text_bg.';"><tr><td align="center"><a href="'.$clickurl.'" style="color:#'.$text_clr.'">'.$text_title.'</a></td></tr></table>';
	} else {
		$this_size = getimagesize($imageurl);
		$this_width = $this_size[0];
		$this_height = $this_size[1];
		echo '<p style="text-align:center;"><a	href="'.$clickurl.'"><img src="'.$imageurl.'" width="'.$this_width.'" height="'.$this_height.'" alt="" /></a></p>';
	}
...
function show_all_banners($act=0) {
	...
	if ($db->sql_numrows($result)) {
		echo '<br />';
		OpenTable();
		echo '<p style="text-align:center;">'._ALL.' '._BANNERS.'</p><p style="text-align:center;">';
		while ($row = $db->sql_fetchrow($result)) {
...
function make_banner_form() {
	...
	<form method="post" action="'.URL::index('&amp;op=make_new').'" enctype="multipart/form-data" accept-charset="utf-8">
...
if ($user_id = is_user()) {
	if (isset($_GET['op']) && $_GET['op'] == 'make_new' && isset($_POST['type'])) {
		...
		$db->sql_query('INSERT INTO '.$prefix.'_banner (bid, cid, imptotal, impmade, clicks, imageurl, clickurl, alttext, date, dateend, type, active, textban, text_width, text_height, text_title, text_bg, text_clr) '.
				"VALUES (DEFAULT, '$user_id', '$imptotal', '0', '0', '$imageurl', '$clickurl', '$alttext', ".time().", '0', '$type', '0', '$textban', '$text_width', '$text_height', '$text_title', '$text_bg','$text_clr')");
	...
	require_once('header.php');
	OpenTable();
	echo '<h3 style="text-align:center;">You must <a href="'.URL::index('Your_Account&amp;file=register').'">register</a> before you can create advertisements!</h2>';
	CloseTable();
	show_all_banners(1);
}