...
if ($sa == 'show_month') {
	...
	while (list($sid, $catid, $title, $time, $comments, $counter, $topic, $alanguage, $score, $ratings) = $db->sql_fetchrow($result)) {
		$actions = '<a href="'.URL::index("News&amp;file=print&amp;sid=$sid").'"><img src="images/news/print.gif" alt="'._PRINTER.'" title="'._PRINTER.'" /></a>
		<a href="'.URL::index('News&amp;file=friend&amp;sid='.$sid).'"><img src="images/news/friend.gif" alt="'._FRIEND.'" title="'._FRIEND.'" /></a>';
		$rated = ($score != 0) ? substr($score / $ratings, 0, 4) : 0;
		if ($catid == 0) {
			$title = '<a href="'.URL::index('News&amp;file=article&amp;sid='.$sid)."\">$title</a>";
		} elseif ($catid != 0) {
			list($cat_title) = $db->sql_ufetchrow("SELECT title FROM ".$prefix."_stories_cat WHERE catid='$catid'", SQL_NUM);
			$title = '<a href="'.URL::index('News&amp;catid='.$catid).'"><i>'.$cat_title.'</i></a>: <a href="'.URL::index('News&amp;file=article&amp;sid='.$sid).'">'.$title.'</a>';
		}
		if ($MAIN_CFG['global']['multilingual']) {
			if ($alanguage == '') { $alanguage = $MAIN_CFG['global']['language']; }
	...
	<form action="'.URL::index('Search').'" method="post" enctype="multipart/form-data" accept-charset="utf-8"><div>
	<input type="text" name="search" size="30" maxlength="255" />&nbsp;
	<input type="hidden" name="modules[]" value="News" />
	<input type="submit" value="'._SEARCH.'" />
	</div></form>
	[ <a href="'.URL::index().'">'._ARCHIVESINDEX.'</a> | <a href="'.URL::index('&amp;sa=show_all').'">'._SHOWALLSTORIES.'</a> ]</div>';
	CloseTable();
}
else if ($sa == 'show_all') {
	...
	if ($numrows > 250 && $min == 0) {
		$min = $min+250;
		$a++;
		echo '<div style="text-align:center;">[ <a href="'.URL::index("&amp;sa=show_all&amp;min=$min").'">'._NEXTPAGE.'</a> ]</div><br />';
	}
	if ($numrows > 250 && $min >= 250 && $a != 1) {
		$pmin = $min-250;
		$min = $min+250;
		$a++;
		echo '<div style="text-align:center;">[ <a href="'.URL::index('&amp;sa=show_all&amp;min='.$pmin).'">'._PREVIOUSPAGE.'</a> | <a href="'.URL::index('&amp;sa=show_all&amp;min='.$min).'">'._NEXTPAGE.'</a> ]</div><br />';
	}
	if ($numrows <= 250 && $a != 1 && $min != 0) {
		$pmin = $min-250;
		echo '<div style="text-align:center;">[ <a href="'.URL::index('&amp;sa=show_all&amp;min='.$pmin).'">'._PREVIOUSPAGE.'</a> ]</div><br />';
	}
	echo '<hr />
	<span class="content">'._SELECTMONTH2VIEW.'</span><br />'
	.monthlist()
	...
	[ <a href="'.URL::index().'">'._ARCHIVESINDEX.'</a> ]</div>';
	CloseTable();
}
else {
	require_once('header.php');
	...
	[ <a href="'.URL::index('&amp;sa=show_all').'">'._SHOWALLSTORIES.'</a> ]</div>';
	CloseTable();
}
