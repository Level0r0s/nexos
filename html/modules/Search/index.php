...
if (!isset($_POST['search']) && !isset($_GET['search'])) {
	...
	foreach ($modlist as $leftover) {
		if (class_exists($leftover['search_class'])) {
			...
			if ($search->result_count > 0) {
				echo '<li><a href="'.URL::index('Search&amp;search='.$url_query.'&amp;mod='.$leftover['module']).'">'.$search->title.'</a> ('.$search->result_count.' '._SEARCHRESULTS.')</li>';
			}
			unset($search);
		}
	}
	if (is_active('Web_Links')) {
		...
		if ($cnt) echo '<li><a href="'.URL::index('Web_Links&amp;l_op=search&amp;query='.$url_query).'">'._WEBLINKS.'</a> ('.$cnt.' '._SEARCHRESULTS.')</li>';
	}
	
	$url_query = urlencode($MAIN_CFG['global']['sitename'].' '.$query);
