<?php
/*********************************************
  CPG Dragonfly™ CMS
  ********************************************
  Copyright © 2004-2006 by CPG-Nuke Dev Team
  http://dragonflycms.org

  Dragonfly is released under the terms and conditions
  of the GNU GPL version 2 or any later version

  $Source: /cvs/l10n/chinese_traditional/web_links.php,v $
  $Revision: 9.5 $
  $Author: nanocaiordo $
  $Date: 2006/02/26 21:39:50 $
Encoding test: n-array summation ∑ latin ae w/ acute ǽ
*******************************************************/
if (!defined('CPG_NUKE')) { exit; }
global $sitename, $anonwaitdays, $outsidewaitdays;
define('_YOURNAME','您的名字');
define('_CATEGORIES','類別');
define('_LVOTES','投票一覽');
define('_TOTALVOTES','總票數：');
define('_LINKTITLE','鏈結的標題');
define('_THEREARE','有');
define('_NOMATCHES','找不到與您的詢問條件相符的資料');
define('_SCOMMENTS','評論');
define('_ADDLINK','增加鏈結');
define('_POPULAR','受歡迎的');
define('_TOPRATED','最高分的');
define('_RANDOM','隨機的');
define('_LINKSMAIN','主要鏈結');
define('_LINKCOMMENTS','鏈結評註');
define('_ADDITIONALDET','附加細節');
define('_EDITORREVIEW','編輯評論');
define('_REPORTBROKEN','失效鏈結報告');
define('_LINKSMAINCAT','網站鏈結主類別');
define('_AND','和');
define('_INDB','在我們的資料庫');
define('_ADDALINK','新增鏈結');
define('_INSTRUCTIONS','指示');
define('_SUBMITONCE','一個鏈結只能投遞一次');
define('_POSTPENDING','張貼的鏈結都要等待確認');
define('_USERANDIP','您的姓名和IP會被紀錄，請勿濫用本系統');
define('_PAGETITLE','標題');
define('_PAGEURL','網址');
define('_YOUREMAIL','您的電子郵件');
define('_LDESCRIPTION','描述： (最多255個字)');
define('_ADDURL','增加這個網址');
define('_LINKSNOTUSER1','您尚未成為登記使用者或你沒有登入');
define('_LINKSNOTUSER2','若您已在本站登記，才能新增網站鏈結');
define('_LINKSNOTUSER3','成為一個註冊使用者是一件非常快速而容易的事');
define('_LINKSNOTUSER4','為什麼我們會對某些功能要求註冊資格呢？');
define('_LINKSNOTUSER5','為的是提供您更高品質的內容，');
define('_LINKSNOTUSER6','每一個項目都個別地被我們檢查與驗證');
define('_LINKSNOTUSER7','我們希望提供您真正有價值的資訊。');
define('_LINKSNOTUSER8','<a href="'.getlink("Your_Account").'">請來註冊帳號</a>');
define('_LINKALREADYEXT','錯誤：本網址已存在於資料庫！');
define('_LINKNOTITLE','錯誤：您必須為您的網址輸入一個標題！');
define('_LINKNOURL','錯誤：您必須為您的網址輸入一個網址！');
define('_LINKNODESC','錯誤：您必須為您的網址輸入一些描述！');
define('_LINKRECEIVED','我們已收到了您所投遞的連結，多謝！');
define('_EMAILWHENADD','在我們認可後您將收到電子郵件');
define('_CHECKFORIT','未提供您的電子郵件信箱，不論如何，我們會盡快檢查您的網站連結');
define('_NEWLINKS','最新的鏈結');
define('_TOTALNEWLINKS','全部最新的鏈結');
define('_LASTWEEK','最近一周');
define('_LAST30DAYS','最近卅天');
define('_1WEEK','1 周');
define('_2WEEKS','2 周');
define('_30DAYS','30 天');
define('_TOTALFORLAST','全部最新的鏈結');
define('_ADDEDON','增加於');
define('_RATING','評分');
define('_RATESITE','對這個網站評分');
define('_DETAILS','細節');
define('_BESTRATED','最高分的鏈結');
define('_OF','之');
define('_TRATEDLINKS','全部已評分的鏈結');
define('_TVOTESREQ','最少投票需要');
define('_SHOWTOP','排行顯示');
define('_MOSTPOPULAR','最受歡迎之');
define('_OFALL','全部有');
define('_SORTLINKSBY','排序鏈結,依');
define('_SITESSORTED','目前網站排序是依');
define('_POPULARITY','受歡迎程度');
define('_SELECTPAGE','選擇網頁');
define('_MAIN','主要的');
define('_NEWTODAY','今日的新連結');
define('_NEWLAST3DAYS','最近3天新的連結');
define('_NEWTHISWEEK','本週新的連結');
define('_CATNEWTODAY','在今日內，本類別新增的鏈結');
define('_CATLAST3DAYS','最近三天內，本類別新增的鏈結');
define('_CATTHISWEEK','在本週內，本類別新增的鏈結');
define('_POPULARITY1','流行度 (點閱數由小至大)');
define('_POPULARITY2','流行度 (點閱數由大至小)');
define('_TITLEAZ','標題 (A to Z)');
define('_TITLEZA','標題 (Z to A)');
define('_DATE1','日期 (舊的鏈結先)');
define('_DATE2','日期 (新的鏈結先)');
define('_RATING1','評分 (低分至高分)');
define('_RATING2','評分 (高分至低分)');
define('_SEARCHRESULTS4','搜尋結果,對');
define('_USUBCATEGORIES','子類別');
define('_LINKS','鏈結');
define('_TRY2SEARCH','嘗試搜尋');
define('_INOTHERSENGINES','在其他搜尋引擎');
define('_EDITORIAL','主編評論');
define('_LINKPROFILE','鏈結的基本資料');
define('_EDITORIALBY','主編者');
define('_NOEDITORIAL','這個網站目前尚無相關主編評論');
define('_VISITTHISSITE','拜訪網站');
define('_RATETHISSITE','評分這個網站');
define('_ISTHISYOURSITE','這是您提供的網站連結嗎？');
define('_ALLOWTORATE','允許其他網站連結到本站的網站評分功能！');
define('_LINKRATINGDET','鏈結評分');
define('_OVERALLRATING','總評分');
define('_TOTALOF','全部共');
define('_USER','使用者');
define('_USERAVGRATING','使用者平均分數');
define('_NUMRATINGS','評分次數');
define('_EDITTHISLINK','編輯這個鏈結');
define('_REGISTEREDUSERS','已註冊的會員');
define('_NUMBEROFRATINGS','評分數目');
define('_NOREGUSERSVOTES','沒有註冊者投票');
define('_BREAKDOWNBYVAL','評分以數值分類');
define('_LTOTALVOTES','全部投票');
define('_LINKRATING','鏈結評分');
define('_HIGHRATING','高評分');
define('_LOWRATING','低評分');
define('_NUMOFCOMMENTS','評註數目');
define('_WEIGHNOTE','* 註：這個網站連結加重會員與訪客評分');
define('_NOUNREGUSERSVOTES','訪客無法投票');
define('_WEIGHOUTNOTE','* 註：這個網站連結加重會員與其它網站投票者評分');
define('_NOOUTSIDEVOTES','無站外投票');
define('_OUTSIDEVOTERS','站外投票');
define('_UNREGISTEREDUSERS','非註冊使用者');
define('_PROMOTEYOURSITE','宣傳您的網站');
define('_PROMOTE01','也許您會對我們所提供的幾種遠端\'評分網站\'的選項感到興趣。這讓您可以用圖片(或甚至一個評分格式)放在您的網站。以增加您的網站接收到的投票數。請選擇下列選項：');
define('_TEXTLINK','文字方式鏈結');
define('_PROMOTE02','一種對鏈結的評分格式，就是簡單地使用文字方式顯示鏈結：');
define('_HTMLCODE1','本情況中，您可使用的HTML格式如下：');
define('_THENUMBER','數字');
define('_IDREFER','位在HTML語法中在 '.$sitename.' 資料庫參照您網站的識別碼，。請確定數字與現況相符。');
define('_BUTTONLINK','按鍵方式鏈結');
define('_PROMOTE03','除了用基本的文字方式顯示鏈結, 您還可以使用按鍵方式鏈結：');
define('_RATEIT','評分此網站！');
define('_HTMLCODE2','上述按鍵的原始碼為：');
define('_REMOTEFORM','遠端評分格式');
define('_PROMOTE04','若您作弊，我們將移除您的鏈結。這是目前的評分格式看起來的樣子');
define('_VOTE4THISSITE','投票給這個網站！');
define('_LINKVOTE','投票！');
define('_HTMLCODE3','本表單可讓使用者直接在您的網站對您的網站連結評分而且評分會被紀錄於此。請將下列原始碼貼至您的網站的網頁即可使用：');
define('_PROMOTE05','謝謝，祝您幸運！');
define('_THANKSBROKEN','感謝您協助維護此目錄之完整。');
define('_THANKSFORINFO','感謝提供資訊');
define('_LOOKTOREQUEST','我們會立刻處理您的請求');
define('_ONLYREGUSERSMODIFY','只有註冊使用者能建議下載資源的修改，請<a href="'.getlink("Your_Account").'">註冊或登入</a>.');
define('_REQUESTLINKMOD','要求鏈結修改');
define('_LINKID','鏈結識別碼');
define('_SENDREQUEST','寄出請求');
define('_THANKSTOTAKETIME','感謝您花時間在此對一個網站評分');
define('_LETSDECIDE','使用者的意見可以幫助其他的使用者來判斷，哪一個鏈結對他們有用');
define('_RETURNTO','回到');
define('_RATENOTE1','請不要對同一個連結投一票以上');
define('_RATENOTE2','分數由1分到10分, 1 為待改進 10 為極優良');
define('_RATENOTE3','請客觀地投票, 假如每一人都投1或10, 評分就不具意義');
define('_RATENOTE4','要看清單請到 <a href="'.getlink("Web_Links&amp;l_op=TopRated").'">網站評分排行榜</a>.');
define('_RATENOTE5','請不要投給您自有的資源或是競爭者');
define('_YOUAREREGGED','您是一個已登入的註冊使用者');
define('_FEELFREE2ADD','請自由地評論這個網站.');
define('_YOUARENOTREGGED','您不是一個註冊使用者或您尚未登入');
define('_IFYOUWEREREG','會員可作此網站的評論');
define('_MODIFY','修改');
define('_COMPLETEVOTE1','感謝您的投票.');
define('_COMPLETEVOTE2','您在過去的 '.$anonwaitdays.' 天之內, 已經對這個網站連結投票過了.');
define('_COMPLETEVOTE3','只能對這個網站投一次票.<br>所有的投票已經紀錄下來, 且經過檢查了.');
define('_COMPLETEVOTE4','您不能對您自己登錄的連結投票<br>所有的投票已經紀錄下來, 且經過檢查了.');
define('_COMPLETEVOTE5','沒選擇票選功能,不舉行票選');
define('_COMPLETEVOTE6','每個 IP 位址在 '.$outsidewaitdays.' 天之內只允許投一次票.');

/**************************
  ADMNINISTRATION SECTION
**************************/
define('_ADDMAINCATEGORY','Add a MAIN Category');
define('_ADDSUBCATEGORY','Add a SUB-Category');
define('_WEBLINKSADMIN','Web Links Administration');
define('_EZTRANSFERLINKS','Transfer all links from category');
define('_CLEANLINKSDB','Clean Links Votes');
define('_EZTRANSFER','Transfer');
define('_BROKENLINKSREP','Broken Links Reports');
define('_LINKMODREQUEST','Link Modification Requests');
define('_ADDNEWLINK','Add a New Link');
define('_MODLINK','Modify a Link');
define('_LINKSINDB','Links in our Database');
define('_LINKSWAITINGVAL','Links Waiting for Validation');
define('_ERRORURLEXIST','ERROR: This URL is already listed in the Database!');
define('_ERRORNOTITLE','ERROR: You need to type a TITLE for your URL!');
define('_ERRORNOURL','ERROR: You need to type a URL for your URL!');
define('_ERRORNODESCRIPTION','ERROR: You need to type a DESCRIPTION for your URL!');
define('_NEWLINKADDED','New Link added to the Database');
define('_YOURLINKAT','Your Link at');
define('_LINKVALIDATION','Link Validation');
define('_CHECKALLLINKS','Check ALL Links');
//define('_VALIDATELINKS','Validate Links');
//define('_FAILED','Failed!');
//define('_BEPATIENT','(please be patient)');
//define('_VALIDATINGCAT','Validating Category (and all subcategories)');
//define('_VALIDATINGSUBCAT','Validating Sub-Category');
define("_ADDEDITORIAL","Add Editorial");
define("_EDITORIALTITLE","Editorial Title");
define("_EDITORIALTEXT","Editorial Text");
define("_DATEWRITTEN","Date Written");
define("_NOREPORTEDBROKEN","No reported broken links.");
define("_USERREPBROKEN","User Reported Broken Links");
define("_IGNOREINFO","Ignore (Deletes all <b><i>requests</i></b> for a given link)");
define("_DELETEINFO","Delete (Deletes <b><i>broken link</i></b> and <b><i>requests</i></b> for a given link)");
define("_LINK","Link");
define("_LINKOWNER","Link Owner");
define("_USERMODREQUEST","User Link Modification Requests");
define("_ORIGINAL","Original");
define("_PROPOSED","Proposed");
define("_NOMODREQUESTS","There are not any modification requests right now");
