<?php
/*********************************************
  CPG Dragonfly™ CMS
  ********************************************
  Copyright © 2004-2006 by CPG-Nuke Dev Team
  http://dragonflycms.org

  Dragonfly is released under the terms and conditions
  of the GNU GPL version 2 or any later version

  $Source: /cvs/l10n/castellano/news.php,v $
  $Revision: 1.6 $
  $Author: estlane $
  $Date: 2009/02/26 16:57:10 $
Encoding test: n-array summation ∑ latin ae w/ acute ǽ
*******************************************************/
if (!defined('CPG_NUKE')) { exit; }
//define('_PRINTER','Versión Imprimible');
define('_FRIEND','Enviar a un Amigo');
define('_YOURNAME','Tu nombre');
define('_RELATED','Enlaces Relacionados');
define('_MOREABOUT','Más Acerca de');
define('_NEWSBY','Noticias de');
define('_MOSTREAD','Noticia más leída sobre');
define('_READMORE','Leer más...');
define('_BYTESMORE','bytes más');
define('_COMMENTSQ','¿Comentarios?');
define('_COMMENT','Comentario');
define('_CONFIGURE','Configurar');
define('_LOGINCREATE','Entrar/Crear Cuenta');
define('_THRESHOLD','Puntos');
define('_NOCOMMENTS','Sin Comentarios');
define('_NESTED','Anidados');
define('_FLAT','Plano');
define('_THREAD','Discusión');
define('_OLDEST','Viejos Primero');
define('_NEWEST','Nuevos Primero');
define('_HIGHEST','Mayor Puntuación Primero');
define('_COMMENTSWARNING','Los comentarios son propiedad de quien los envió. No somos responsables de su contenido.');
define('_SCORE','Puntuación');
define('_USERINFO','Información del Usuario');
define('_READREST','Leer el resto del comentario...');
define('_REPLY','Responder');
define('_REPLYMAIN','Enviar Comentario');
define('_NOSUBJECT','Sin Título');
define('_NOANONCOMMENTS','No se permiten comentarios Anónimos, <a href="modules.php?name=Your_Account&amp;op=new_user">Restregaseis</a> por favor');
define('_PARENT','Padre');
define('_ROOT','Raíz');
define('_UCOMMENT','Comentario');
define('_ALLOWEDHTML','HTML Permitido:');
define('_POSTANON','Envío Anónimo');
define('_EXTRANS','Extrans (html tags a texto)');
define('_HTMLFORMATED','Formateado HTML');
define('_PLAINTEXT','Texto Plano');
define('_ONN','en...');
define('_COMMENTREPLY','Enviar Comentario');
define('_COMREPLYPRE','Vista Previa del Comentario');
define('_NOTRIGHT','Algo no ha funcionado al pasar una variable a su función. Este mensaje es sólo para dejar claro que hubo un fallo');
define('_SENDAMSG','Enviar un Mensaje');
define('_YOUSENDSTORY','Enviarás la noticia');
define('_TOAFRIEND','a un amigo:');
define('_FYOURNAME','Tu Nombre:');
define('_FYOUREMAIL','Tu Correo:');
define('_FFRIENDNAME','Nombre de su amigo:');
define('_FFRIENDEMAIL','Correo de su amigo:');
define('_INTERESTING','Artículo interesante en');
define('_YOURFRIEND','Tu amigo');
define('_CONSIDERED','consideró interesante el siguiente artículo y quiso enviártelo.');
define('_FDATE','Fecha');
define('_FTOPIC','Tema');
define('_YOUCANREAD','Puedes leer artículos interesantes en');
define('_FSTORY','Noticia');
define('_HASSENT','ha sido enviada a');
define('_THANKS','¡Gracias!');
define('_RECOMMEND','Recomendar este sitio a un amigo');
define('_PDATE','Fecha');
define('_PTOPIC','Tema');
define('_COMESFROM','Este artículo proviene de');
define('_THEURL','La dirección de esta noticia es:');
define('_NEWUSER','Nuevo Usuario');
define('_OPTIONS','Opciones');
define('_REFRESH','Recargar');
define('_NOCOMMENTSACT','Disculpa, los comentarios no están activados para esta noticia.');
define('_ARTICLEPOLL','Encuesta de Artículo');
define('_RATEARTICLE','Votos del Artículo');
define('_RATETHISARTICLE','Por favor tómate un segundo y vota por este artículo:');
define('_CASTMYVOTE','Grabar mi Voto!');
define('_AVERAGESCORE','Puntuación Media');
define('_BAD','Malo');
define('_REGULAR','Regular');
define('_GOOD','Bueno');
define('_VERYGOOD','Muy Bueno');
define('_EXCELLENT','Excelente');
define('_ARTICLERATING','Valoración del Artículo');
define('_THANKSVOTEARTICLE','Gracias por votar por este artículo!');
define('_ALREADYVOTEDARTICLE','Disculpa, ya votaste por este artículo recientemente!');
define('_BACKTOARTICLEPAGE','Regresar a la Página del Artículo');
define('_DIDNTRATE','No seleccionaste ningún valor en la votación de este artículo!');
define('_NOINFO4TOPIC','Disculpa, no existe información para el tema seleccionado.');
define('_GOTONEWSINDEX','Ir al Índice de Noticias');
define('_SELECTNEWTOPIC','Selecciona un nuevo Tema');
define('_GOTOHOME','Ir al Inicio');
define('_SEARCHONTOPIC','Buscar en este Tema');
define('_SEARCHDIS','Buscar Mensaje');

/**************************
  ADMNINISTRATION SECTION
**************************/
define('_ARTICLEADMIN','Administración de Artículos/Noticias');
define('_ADDARTICLE','Añadir Artículo');
define('_EDITARTICLE','Editar Artículo');
define('_USERPROFILE','Perfil de Usuario');
define('_EMAILUSER','Enviar Correo');
define('_SENDPM','Enviar Mensaje Privado');
define('_SELECTTOPIC','Escoja Tema');
define('_PUBLISHINHOME','¿Publicar en el Inicio?');
define('_ACTIVATECOMMENTS','¿Activar Comentarios para esta Noticia?');
define('_DISPLAY_ORDER','Display Order');
define('_NORMAL','Normal');
define('_TOP_MOST','Top Most');
define('_STORYTEXT','Texto de la Noticia');
define('_ARESUREURL','(¿Estás seguro de haber incluido una URL? ¿Has revisado la ortografía?)');
define('_NOTES','Notas');
define('_PROGRAMSTORY','¿Quieres programar esta noticia?');
define('_NOWIS','Ahora es');
define('_HAS','tiene');
define('_DELETESTORY','Borrar Noticia');
define('_PREVIEWSTORY','Vista Previa');
define('_POSTSTORY','Enviar Noticia');
define('_CATEGORIESADMIN','Administración de Categorías');
define('_CATEGORYADD','Añadir Categoría');
define('_CATEGORYNAME','Nombre de la Categoría');
define('_ASELECTCATEGORY','Seleccionar Categoría');
define('_NOARTCATEDIT','No puedes editar la categoría <i>Artículos</i>');
define('_DELETECATEGORY','Borrar Categoría');
define('_SELECTCATDEL','Seleccionar Categoría a Borrar');
define('_CATDELETED','Categoría Borrada');
define('_NOTAUTHORIZED1','No estás autorizado para modificar este Artículo');
define('_NOTAUTHORIZED2','No puedes editar y/o borrar artículos que no has publicado');
define('_REMOVESTORY','¿Estás seguro de que desea borrar la Noticia ID #');
define('_AUTOSTORYEDIT','Editar Historia Automática');
define('_CHNGPROGRAMSTORY','Seleccionar nueva fecha para esta Noticia:');
define('_GOTOADMIN','Ir a la Sección de Administración');
define('_ALLSTORIES','TODAS las Noticias');
define('_STORIESINSIDE','noticias englobadas');
define('_DELCATWARNING1','Puedes Borrar esta Categoría y TODAS sus noticias y comentarios');
define('_DELCATWARNING2','o puedes Mover TODAS las noticias a una Nueva Categoría.');
define('_DELCATWARNING3','¿Qué deseas hacer?');
define('_YESDEL','¡Sí!, Borrar TODO');
define('_NOMOVE','¡No!, Mover las Noticias');
define('_MOVESTORIES','Mover Noticias a una Nueva Categoría');
define('_MOVEDONE','El cambio ha sido realizado.');
define('_WILLBEMOVED','serán movidas.');
define('_SUBMISSIONSADMIN','Administración de Noticias Enviadas');
define('_NEWSUBMISSIONS','Envíos de Nuevas Noticias');
define('_NOSUBMISSIONS','No hay nuevos envíos');
define('_ATTACHAPOLL','Pegar una Encuesta a este Artículo');
define('_LEAVEBLANKTONOTATTACH','(Deja en blanco para publicar el artículo sin encuesta)<br>(NOTA: Noticias Programadas no pueden tener Encuestas Pegadas)');

define('_CATADDED','Nueva Categoría Añadida');
