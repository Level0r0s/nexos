 ...
 *
 *   $Source: /cvs/l10n/korean/Forums/lang_bbcode.php,v $
 *   $Revision: 9.2 $
 *   $Author: djmaze $
...
$faq[] = array("--", "링크 만들기");
$faq[] = array("다른 사이트 링크", "phpBB의 BBCode는 URI(URL로 더 알려진 Uniform Resource Indicators) 만드는 여러 방법을 지원한다.<ul><li>첫번째 방법은 <b>[url=][/url]</b> 태그를 이용하는데, = 기호 다음에 오는 모든 문자들은 URL로 취급된다. 예를 들어 phpBB.com 으로 링크하려면:<br /><br /><b>[url=http://phpbb.com/]</b>phpBB로!<b>[/url]</b><br /><br />다음과 같은 링크를 얻는다, <a href=\"http://phpbb.com/\" target=\"_blank\">phpBB로!</a> 링크가 새로운 창에서 열리므로 게시판은 계속 사용할 수 있다.</li><li>URL 자체를 링크로 표시하려면 :<br /><br /><b>[url]</b>http://phpbb.com/<b>[/url]</b><br /><br />다음과 같은 결과를 얻는다, <a href=\"http://phpbb.com/\" target=\"_blank\">http://phpbb.com/</a></li><li>또한 phpBB 는 <i>Magic Links</i>라는 기능을 제공하는데, 이것은 태그나 http://를 지정하지 않아도 문법적으로 올바른 URL을 링크로 만들어 준다. 예를 들어 메시지내에 phpbb.com 를 입력하면 자동으로 <a href=\"http://phpbb.com/\" target=\"_blank\">phpbb.com</a> 가 메시지에 만들어진다.</li><li>같은 기능이 이메일 주소에도 적용되는데, 다음과 같이 주소를 지정할 수 있다:<br /><br /><b>[email]</b>no.one@domain.adr<b>[/email]</b><br /><br />결과는 다음과 같다 :  <a href=\"emailto:no.one@domain.adr\">no.one@domain.adr</a>,혹은 그냥 no.one@domain.adr 을 메시지에 입력하면 나중에 보기할때 자동으로 변환이 된다.</li></ul>다른 BBCode 태그들과 마찬가지로 <b>[img][/img]</b> (다음 예제 참조), <b>[b][/b]</b>와 같은 태그도 URL을 포함할 수 있다. 포매팅 태그에서 처럼 태그의 열고 닫음을 정확히 해야 한다, 예를 들어:<br /><br /><b>[url=http://phpbb.com/][img]</b>http://phpbb.com/images/phplogo.gif<b>[/url][/img]</b><br /><br />는 잘못된 것이다.");

$faq[] = array("--", "게시물에 이미지 보이기");
$faq[] = array("게시물에 이미지 붙이기", "phpBB의 BBCode는 게시물에 이미지를 넣을수 있도록 태그를 제공한다. 이 태그를 사용할때 기억해야할 두가지 중요한 것이 있는데; 우선 많은 사람들이 이미지가 뜨는 것을 반기지 않는다는 것과 이미지가 이미 인터넷상에 존재하고 있어야 한다(웹서버를 돌리지 않는한, 이미지가 자신의 컴안에 있어서만은 안된다). 현재 phpBB내에 이미지를 저장하는 방법은 없다(미래의 phpBB에서는 고려가 될 것이다). 이미지를 넣으려면 이미지의 URL을 <b>[img][/img]</b> 태그 안에 넣는다. 예를 들어:<br /><br /><b>[img]</b>http://phpbb.com/images/phplogo.gif<b>[/img]</b><br /><br />위에서 언급한 바와 같이 이미지를 <b>[url][/url]</b> 태그안에 넣어도 된다, 예를 들어<br /><br /><b>[url=http://phpbb.com/][img]</b>http://phpbb.com/images/phplogo.gif<b>[/img][/url]</b><br /><br />결과는:<br /><br /><a href=\"http://phpbb.com/\" target=\"_blank\"><img src=\"templates/subSilver/images/logo_phpBB_med.gif\" border=\"0\" alt=\"\" /></a><br />");
